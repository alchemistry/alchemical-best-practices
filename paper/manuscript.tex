...
Equilibrium free energy calculations, solve the problem of having poor overlap between the states of interest by introducing multiple intermediate alchemical states at values $\vec{\lambda}_A = \vec{\lambda}_0, \vec{\lambda}_1, \cdots, \vec{\lambda}_K = \vec{\lambda}_B$ so that each pair of consecutive states $\vec{\lambda}_k, \vec{\lambda}_{k+1}$ share good overlap, meaning that a moderate amount of samples occur with similar probabilities in both simulations. We make "good overlap" more quantitative in section~\ref{sec:data_analysis}. 
...
related ligand as a starting point. Additional factors such as slow protein or ligand rearrangements, uncertainties in ligand binding mode, or charged ligands can make these calculations far less reliable and more of a research effort. A recent study from Open Free Energy showed that the accuracy obtained across a set of private industrial targets (weighted RMSE 1.7 [1.5, 2.0] kcal mol$^{-1}$) was substantially larger than that obtained for common benchmark systems (weighted RMSE 2.4 [1.9, 3.1] kcal mol$^{-1}$), highlighting the increased challenge of real-world application of these methods~\cite{baumann2025largescale}.
