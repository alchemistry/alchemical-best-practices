...
\todo[inline]{JDC: Should we give a few examples, along with illustrative figures?}
...
In this review, we assume a basic familiarity with the principles of molecular mechanics, molecular dynamics simulations, statistical mechanics, and the biophysics of protein-ligand association.
...
\label{sec:step0}
\todo[inline]{Huafeng Xu please edit and comment}
%\begin{itemize}
...
Current alchemical free energy calculations seem to achieve, in favorable cases, RMS errors around 1-2 kcal/mol depending on force field, system, and a variety of other factors such as simulation time, sampling method, and whether the calculations employed are absolute or relative. However, the domain of applicability is a significant concern [cite Sherborne, Cournia], especially for relative calculations, which typically require a bound structure of a closely
...
Thus, Step 0 is essentially about initially counting the cost of your
...
If you don't know whether your project is feasible, one option may be
...
\section{Step 1 -- Simulation prerequisites}
\label{sec:step1}
\todo[inline]{ASJSM: review first pass from JM}
...
For binding free energy calculations the ligand should normally be positioned in its putative binding site in an energetically reasonable conformation that entails no steric or electrostatic mismatch with the receptor. Attention should be paid to binding site water molecules that may form water mediated protein-ligand interactions, particularly in situations where exchange with bulk water may be slow on the simulation timescales. This happens typically in buried binding site. In some implementation water sampling may be facilitated by the use of Grand Canonical Monte Carlo methods, but finite sampling limitations may prevent sampling of the correct solvent distributions. Information about water positions may be obtained from inspection of experimental data, or computed using a variety of molecular modelling methodologies (REFS). 
If the binding site location is uncertain but the structure of the receptor is well defined and plausible binding sites are apparent, absolute free energy calculations may be used to compute the standard free energy of binding of the ligand to a set of binding sites. This requires the user to prepare input files describing the bound conformation in different putative binding sites (REF). The apparent binding free energy of the ligand may be obtained by combining the individual binding site free energies, which also indicate where the ligand is more likely to bind.
...
For relative binding free energy calculations of congeneric series it is rare that experimental data is available to define the bioactive conformation of each analogue. A common assumption is that the binding mode is conserved within a congeneric series and the whole series can be modelled on the basis of a reference structure taken from experimental data or obtained by preliminary calculations. While conservation of binding modes within congeneric series seems to be the case (REFS?), exception exists(REF), and weaker binding compounds are more likely to adopt diverse binding modes (REFS). There may also be ambiguity about the relative orientation of substituents, a typical example are ortho and meta substituted 6-membered aromatic rings. While both positions are formally equivalent, a 180 degree flip of the ring may not occur sufficiently frequently during simulations. This situation may be addressed by explicitly modelling different binding modes of the same ligand and combining later computed free energy differences for different binding modes into a relative free energies of binding.  
