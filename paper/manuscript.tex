...
computing partition ($\log P$) or distribution ($\log D$) coefficients between environments (Fig.~\ref{fig:fig_what_is_alchemy} (\texbf{B}))~\cite{rustenburg2016measuring, bosisio2016blinded} or compartments or membranes (Fig.~\ref{fig:fig_what_is_alchemy} (\textbf{C}))~\cite{corey2019insights}. 
...
\begin{equation}
    \Delta G_{\mathrm{bind}} \cong \Delta F= -k_BT\ln\frac{\int_\mathrm{bound} \exp(-\beta U(\mathbf{x}))d\mathbf{x}}{\int_\mathrm{unbound} \exp(-\beta U(\mathbf{x}))d\mathbf{x}},
\end{equation}
