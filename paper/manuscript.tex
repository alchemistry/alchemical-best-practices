...
The calculations use non-physical intermediate states where atoms are changed from one type to another or can be removed or inserted by decoupling or coupling with their environment. 
...
In order to obtain a free energy from these AFE simulations, historically two common approaches were used for the analysis: free energy perturbation (FEP) using the Zwanzig relation~\cite{zwanzig1954hightemperature} or Bennett's acceptance ratio~\cite{bennett1976efficient} and thermodynamic integration (TI) for which the theory dates back decades but with the first computational applications emerging in the 1980's and 90's~\cite{kirkwood1935statistical, jorgensen1985monte, kollman1993free, wong1986dynamics, merz1989free}. Important studies continued during the 2000's often leading to improved implementation of the methods in common simulation software~\cite{vanderspoel2005gromacs, mermelstein2018fast, wang2015accurate, hedges2019biosimspace}. This foundational work, combined with the methodological, technological, and hardware improvements of the last 5-10 years have led to an explosion of interest.

As the field of molecular simulation moves towards millisecond timescales accurate AFE calculations on even more challenging problems will be in scope. In the meantime, today's users may find it hard to get started with these complex calculations whilst also keeping up with the fast pace of change. This best practice guide aims to serve as a go-to reference for current recommendations and tips for users of all experience.  
...
We will cover topics that are key to the preparation~(\ref{sec:step1}), choice and use of correct protocols~(\ref{sec:step2}), and finally the best practices that should be used for the analysis of AFE calculations~(\ref{sec:step3}). 
...
\todo[inline, color={green!20}]{ASJSM: @everyone: Might be nice to do an example calculation X perturbation on GTX 1080s on a AWS cluster cost \$\$X. Synthesis costs \$\$Y. }
%
...
\todo[inline, color={green!20}]{ASJM: This a bit waffly. Can we address: what do softcore potentials actually look like (equation)? Give an example of a lambda switching function(equation)? How should you chose your lambda intermediates? What is the best practice here? I have conveniently provided a figure to help with the discussion.}
...
\todo[inline, color={green!20}]{ASJSM: I'll expand on this}
...
For example, 
convergence of relative free energy calculations in predictive simulations where the entire phase space is not known in advance, requires sampling the different kinetically stable states \todo[inline]{cite Mobley and Klimovich JCP 2012}. 
...
To minimize this effect, an initial portion of the simulation is often discarded to \emph{equilibration}~\cite{https://doi.org/10.33011/livecoms.1.1.5957}, with the idea of removing the most heavily biased initial portion of simulation data but retaining the unbiased \emph{production} region that represents a stationary Markov chain process sampling from the desired equilibrium target distribution.
...
For additional discussion of working with correlated data and autocorrelation analysis, please refer to the work on Best Practices for Quantification of Uncertainty and Sampling Quality in Molecular Simulations~\cite{https://doi.org/10.33011/livecoms.1.1.5067}.
...
relationship}~\cite{zwanzig1954hightemperature}, also called one-sided exponential reweighting (EXP), or simply free energy perturbation, thought that term is sometimes used to encompass all ways of calculating free energy differences.
...
require using pathways (such as Lennard-Jones softcore) that are not linear, as we discuss, making this more complex.  Still, if the code of interest does compute $\frac{du}{d\lambda}$, then TI is perhaps the simplest method to use, as it involves a very little post-processing,
...
As with any modelling technique, misuse of statistical analysis can skew the perception of how well models perform in FE predictions. First and foremost, error estimates should always be included on your predictions in whatever form you present your data (scatterplots, barplots, etc; see next paragraph). It is recommended to perform triplicates of your predictions at minimum to ensure some measure of reliability in your data, in part because analytical uncertainty estimates often underestimate true the true statistical uncertainty.

Because of the strong drug discovery context and the relevance of alchemical FE predictions to SAR projects, both the overall correlation of your predictions relative to experiment and the ability of your models to detect the best-scoring compounds (i.e. rankings) should be computed. Conventionally, this means including an R\textsuperscript{2} (or Pearson r) and a Kendall \texttau{} metric in your results. Additionally, practitioners may choose to include a Spearman \textrho{} as well. Practitioners should take particular care with correlation based metrics, however, when  the dataset dynamic range is small, or only a small number of compounds appear at extrema of the dynamic range. Bootstrapping analysis -- including variation in the dataset -- can help with this by providing useful error bars on performance metrics.

Mean unsigned error (MUE, also called mean absolute error/MAE) is the another key statistic to include in your results. Even though some models' near-perfect correlation and ranking statistics might suggest excellent accuracy, MUE values can still be multiple kcal/mol, providing important additional insight into performance. Furthermore, MUE allows for unbiased comparisons between predictive models as it is less sensitive to dataset size. Even though this section may serve as an introduction to the main statistical practices, the authors recommend further reading on evaluation of computational models \cite{cite Nicholls pt1 and pt2 + Jain&Nicholls "Recommendations for.."}.
...
\todo[inline, color={green!20}]{ASJSM:This figure needs to find a better home in the text}
\begin{figure}
...
\end{itemize}
\end{itemize}
\todo[inline]{JS: should this be combined with the acronyms section at the beginning of the paper?}
%%%%%%%%%%%%%%%%%%%%
%                Software                    %
%%%%%%%%%%%%%%%%%%%%
...
\begin{adjustwidth}{-1cm}{}           % workaround to make the dual box pass the margin
\begin{todolist}
    \begin{tcolorbox}
    % absolute
...
\begin{tcolorbox}[width=15cm, center]
    ...
    \item Compute both correlation and ranking coefficients and error statistics (e.g. r, \textrho, MUE and \texttau)
