...
Consider a molecular dynamics trajectory $Z (t ; z_0)$ started at some initial configuration $z_0 = (x_0, v_0)$, where $x_0 \in \mathbb{R}^{3 N}$ denotes the initial positions of the system and $v_0 \in \mathbb{R}^{3 N}$ its initial velocities. If $z = (x_0, v_0)$ is drawn from some initial density $\rho_0 (z)$, the density time $t$ will be
\begin{equation}
...
One should not confuse a plateau in such coordinates with having explored the entirety of the configuration space $\Omega$; most biomolecular systems, even ones involving only a single ligand, possess a whole hierarchy of timescales, the longest of which can easily exceed achievable simulation times. Importantly, there is no that $\rho_t (x | x_0)$ converges as $t \rightarrow \infty$ to some unique invariant distribution $\propto \exp (-\beta H (x))$, nor that $p_t (x | x_0)$ is invariant to $x_0$.

% The following footnote feels out of place, but might address some comments by A. Bhati re: the Poincaré recurrence theorem.
% \footnote{(Luckily, it is not necessary for simulations to visit every configuration $x \in \Omega$ in order to exhibit a regime of convergence. For instance, two configurations $x$ and $x'$ may be considered equivalent if they differ only by the permutation of water molecules. Nature too faces the same difficulty that not all states are accessible on timescales we can observe -- it will take an astronomically long time before all the gas particles in a room happen to be on one half-side of it -- but this does not prevent us from measuring the temperature of (most) rooms.)}

For systems involving a protein-ligand complex, it is usually the case that $\rho_t (x | x_0)$ is sensitive to $x_0$ and additionally may not exhibit a clear plateau on any accessible time $T > 0$ for "most" $x_0$. In such a case, we can still draw independent samples from $\rho_t (\cdot | x_0)$ by simulating a trajectory up to time $t$ and taking the last frame, but this is much less valuable than knowing that $\rho_t (\cdot | x_0) \approx \rho_T (\cdot | x_0)$ for all $t \in [T_{\min}, T]$.\footnote{Should one define the mixing time of a Markov process? Cite Peres Markov Chains and Mixing Times, which is freely available?}


In this section, we explore the various possible types of error analysis which can be performed, starting from the simplest, with end hope of making "$\rho_t (x | x_0)$ should not vary much'' a little more precise. We start with the following definition.
...
\end{equation}
The above equation says that $S_R^2(t)$ is an unbiased estimator for the variance. As $R \rightarrow \infty$, we have $S^2_R (t) \rightarrow \mathrm{Var} _{x_0}(\Delta
G^{(1)} (t)) = \sigma^2 (t ; x_0)$.
...
(t ; x_0) \rightarrow 0$ as $R \rightarrow \infty$ no matter what $S_R^2 (t)$, so long as it is finite, which is true if the $\overline{\Delta G}$ are, say, bounded.
