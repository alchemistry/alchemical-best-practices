...
Once a single coordinate set approach is selected is selected, a crucial next step is to identify the common atoms which will not be perturbedbetween the two molecules (respectivly end-states).
...
Alternativley an approach where such a restraint algorithm was included in a pipeline for ABFE calculations is described by the ABFE\_workflow \cite{Ries2024ABFE} using MDRestraintsGenerator\cite{alibay2021mdrestraintsgenerator}.
