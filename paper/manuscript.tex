...
\item
\item And finally
\item choice of equilibrium or nonequilibrium methods (probably prefer equilibrium, because easier to check things; nonequilibrium requires checking to make sure the distribution is converged, but you aren't guaranteed you don't miss things.
\end{itemize}
\end{checklist}
...
\end{itemize}
\item Hamiltonian Replica exchange can always help, if you can run the simulations in parallel, as the simulation will be able to sample more in the uncoupled state.
\end{checklist}
...
\item Pick an appropriate method; usually MBAR, BAR, or TI.  Better yet, use multiple types of analysis.
\item Make sure the data you collect are uncorrelated samples.
\item Visualize the molecule to make sure it is behaving as you expect.
\end{checklist}
...
\item Ensure all simulations at least 50x correlation time %MRS: I've heard 40x from Swope. Rationale has to do with the number of uncorrelated samples for the estimate of the uncertainty to be reliable.
\end{itemize}
...
\item If possible, look at multiple estimators.  Significant discrepancies between different free energy estimators (TI, BAR, MBAR) indicate there is a problem..
...
\item Dependence on initial conformation (best practice would be to look at 
...
\begin{itemize}
\item MBAR is gold standard, but requires energies from each sample at all of the other states.  BAR is almost as good if the states are separated so as to minimize the number of states (i.e. there is good overlap from $i \rightarrow i+1$ and $i \rightarrow i-1$, but not to $i\pm 2$.
\item Using bootstrap (or block bootstrap) for errors is more reliable than analytical error estimates. BAR uncertainties are underestimate.
\item TI should be roughly in agreement, but quadrature error hard to quantify; should generally be used only if the path is known to be good for that size of molecule.
\item Other variants useful in special circumstances (e.g. Z. Tan stochastic version)
\item expontential averaging is very problematic.  If using intermediates (which will almost always be the case) should avoid.
\item Looking at the overlap matrix and making sure that there is decent overlap between all states to ensure that there are sufficiently high number of intermediates.
\end{itemize}
\item Computing and reporting uncertainties on free energies
\begin{itemize}
\item Using correlated bootstrap versus timeseries subsampling.
\item Variation in dG estimate using multiple methods (TI, DEXP, IEXP, BAR, MBAR, GDEL, etc.)
...
\begin{itemize}
\item Compute cycle closure error
\end{itemize}
