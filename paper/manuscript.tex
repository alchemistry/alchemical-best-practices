...
We define an \textbf{ensemble} to be a set $\{ X^{(1)}, \ldots, X^{(R)} \}$ of $R \geqslant 1$ trajectories, all started at the same configuration $x_0$, with $v_0$ drawn from the Maxwell-Boltzmann distribution at the appropriate temperature, and run for a chemical time $T$. 
