...
This section outlines the basic principles of uncertainty estimation. The theory is developed using the theory of Markov processes and it is usually assumed that the underlying process is "mixing", which loosely means that the simulations explore the configuration space well enough. As many practitioners have noted [cite], this assumption is not always satisfied. The result is a consistent pattern: when a simulation is well behaved, most reasonable error estimates tend to agree; conversely, when different estimation methods yield substantially different results—which is often the case—it typically indicates that the simulation itself is not well behaved. In such situations, all standard error estimates should be expected to underestimate the true uncertainty.

In this section we will tend not to make definitive statements, and we shall simply provide "best practices" that most practitioners will agree on. In the following subsection, we give a brief explanation of the difficulty. %MS: It may be worthwhile to note that various groups may advocate for different statistical procedures, highlighting the value of many individual trials ("replicates") over one long simulation. This same discussion has played out many times within the Markov Chain Monte Carlo community.; see e.g. https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210X.2011.00131.x
...
\end{equation}
where $\Omega$ denotes the state space. Even though not all estimators can be written in the form \eqref{eqn:empirical-average}, it will be sufficient to consider this case to get the main idea. The key object of study in this section is an estimator of 
\begin{equation}
...
The simplest  and most robust way to obtain error bars on free energy estimates is by simulating an ensemble $\{ X^{(1)}, \ldots, X^{(R)} \}$ of $R$ molecular dynamics trajectories started at $x_0$ and producing trajectories of free energy differences $\{ \Delta G^{(1)}, \ldots, \Delta G^{(R)} \}$. Trajectories $\Delta
G^{(i)} (\cdot)$ are identically distributed and conditionally independent given some $x_0$. For any time $T > 0$, the variance of any $\Delta G^{(i)} (T)$ can be estimated with the sample variance $S^2_{(R - 1)} (T)$,
...
\end{equation}
The above equation says that $S_R^2(T)$ is an unbiased estimator of the variance. By the Law of Large Numbers, as $R \rightarrow \infty$, we have $S^2_R (T) \rightarrow \mathrm{Var} _{x_0}(\Delta
G^{(1)} (T)) = \sigma^2 (T ; x_0)$ and hence
\begin{equation}\label{eqn:variance-of-empirical-mean}
\mathrm{Var}_{x_0}(\overline{\Delta G(T)}) = \frac{\mathrm{Var}_{x_0}(\Delta G^{(1)}(T))}{R}.
\end{equation}
...
\end{equation}
is the gold standard for reporting the error of a free energy estimate $\overline{\Delta G (T)} $. Note that$\mathrm{SE}_R
...
Partition the $N$ samples into $B$ consecutive batches of equal size $M$, so that $N = BM$. Batch $k$ consists of the samples $\{X(t_{kM}),\dots,X(t_{(k+1)M-1})\}, k=0,\dots,B-1.$ Let
\[
...
\]
This estimator may look complicated, but it is just the application of the sample variance to the estimates on  $B$ non-overlapping time blocks of length $M$, which gives the variance using $M$ samples, with an additional normalization by the number of blocks $B$, to get the variance using all $N=BM$ samples. 
...
It is also useful to recall the empirical mean \eqref{eqn:empirical-average} introduced at the beginning of the section. We recall in this notation that $t_{N-1} = T$, and so
\begin{equation}
    \sigma^2(T; x_0) \approx \mathrm{Var}_{x_0}\!\left(\frac{1}{N}\sum_{i=0}^{N-1} Z_i\right),
\end{equation}
where $Z_i = f(X(t_i))$ for a suitable observable $f$. When the process has reached stationarity, the marginal distribution of each $Z_i$ is given by the equilibrium measure $\pi$, and we denote
\[
\sigma_0^2 \equiv \mathrm{Var}_{Z \sim \pi}(Z)
\]
as the equilibrium variance of a single sample. If the $Z_i$ were independent, we would have $\sigma^2(T;x_0) \approx \sigma_0^2/N$. In practice, however, successive samples are correlated, and this increases the variance of the empirical mean. A standard way to account for this is to write
\[
\sigma^2(T;x_0) \approx \frac{\sigma_0^2}{N}\,\tau_{\mathrm{int}},
\]
where $\tau_{\mathrm{int}}$ is the integrated autocorrelation time of the sequence $\{Z_i\}$. Loosely speaking, $\tau_{\mathrm{int}}$ measures the number of indices $i$ between roughly independent samples, and is therefore sometimes referred to as the statistical inefficiency. The role of estimators such as non-overlapping batch means is precisely to estimate $\sigma^2(T;x_0)$ directly from correlated data, which is roughly equivalent to estimating $\tau_{\mathrm{int}}$. There exist much more sophisticated estimators for both $\tau_{\mathrm{int}}$ and $\sigma^2(T;x_0) $, but they are beyond the scope of this article; see (cite)
...
First, the finest discretization one can take for $t_{i+1} - t_i$ is the molecular dynamics simulation timestep $\Delta t_{\mathrm{MD}}$, usually on the order of a few femtoseconds. One rarely stores the trajectory data so finely, because 
\begin{itemize}
...
The best estimators will always be able to make use of all the data, and subsampling can never improve the statistical efficiency of an estimator (so long as it is being correctly used)  \cite{LinkEaton2012,maceachern1994subsampling,geyer1992practical}; sometimes users use the sample variance \eqref{eq:sample-variance-definition} with time-correlated processes, and find that subsampling produces a more representative error estimate -- this is an incorrect use of the sample variance. Subsampling can be useful to balance computational cost or simplify estimation procedures, as might be the case when combining estimates from different replicates or even different ensembles. 

Given a process $\{Z_i\}_{i=0}^{N-1}$ with integrated autocorrelation time $\tau_{\mathrm{int}}$ (or statistical inefficiency), one may form an approximately uncorrelated subsample by retaining every $\tau_{\mathrm{int}}$-th observation. This yields an effective sample size $N_{\mathrm{eff}} \approx N / \tau_{\mathrm{int}}$, corresponding to the number of effectively independent samples. The error 

In many cases, the variance of the free energy estimate is negligibly increased if we only use, say, every $\tau_{\mathrm{int}}/10$ samples. In practice, care should be taken in performing such subsampling, since in general the estimate of $\tau_{\mathrm{int}}$ is poor. 

When simulations are performed independently at each alchemical state $\lambda$, the correlation time may vary significantly across states, and subsampling should be performed separately using a state-specific estimate of $\tau_{\mathrm{int}}$. For coupled simulations, such as Hamiltonian replica exchange, replicas typically undergo similar random walks in alchemical space, and a common subsampling interval may be used across states. Automated equilibration detection methods that maximize the effective sample size often provide a practical estimate of $\tau_{\mathrm{int}}$ over the production region.
