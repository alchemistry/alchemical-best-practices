...
\tocsectioncomment{Steering Meeting: @Emilio Gallicchio to update theory. @Stefan Boresch to comment on dummy atom considerations? Will frame it to be as general as possible, while remaining accessible to beginners. Will align with Gilson et al. 1997 paper. @Darrin York to review.}
\label{sec:theory}
...
\tocsectioncomment{Steering Meeting: @Volunteer Shorten and move most material to second paper. Possibly add a walkthrough of a ``dummy calculation'' (which could be a particular realisation of a general workflow laid out in @Jenke Scheen's figure)? This could lead on to discussion of particular methods.}
\label{sec:step0}
...
\tocsectioncomment{@Hannah Bruce Macdonald/ @David Slowchower/ @Volunteer to expand range of systems discussed, e.g. membrane proteins, cofactors, multimers, PPIs.}
...
\tocsectioncomment{@Volunteer to add best practices for non-equilibrium calculations.}
...
\tocsubsubsectioncomment{@Volunteer: Move parts of this to analysis section (in Single/ Multiple papers).}
