...
Bhati \textit{et al.}~\cite{bhati2017, bieniek2021} presented a variant of the single coordinate dual topology approach which is used in the TIES method. Its distinguishing feature is the application of some filters to remove atoms with differences in atom types and partial charges from the structural MCSS identified. This is followed by ensuring identical charges for all MCSS atoms. The approach ensures that the final MCSS is both structurally and chemically identical, allowing the use of a unique set of MCSS atoms for both ligands and dual topology for their alchemical parts.
...
The readers should note here that replicas within ensemble simulations are independently run (with randomly assigned starting velocities even when the starting structure is the same) and hence generate decorrelated samples by default. Further, Wade \textit{et al.} reported that the underestimation of true uncertainty using one-off simulations remains unresolved even on decorrelating the samples~\cite{wade2022} and hence ensemble simulations are strongly recommended. However, if only a single simulation is available, the procedures below can provide a lower-bound uncertainty estimate.
