...
Determining the correct hydration pattern for the two endpoints may not be sufficient for an accurate and reproducible calculation of relative binding free energies. If the locations of the bound water molecules differ at the two endpoints (e.g., if a perturbation to the ligand displaces a bound water molecule), then the network needs to be able to adapt smoothly during the entire course of the perturbation, with the hydration network having enough time to change smoothly at each step to the correct equilibrium distribution corresponding to that alchemical intermediate.  Otherwise, it is likely that the estimated relative binding free energies will be in error, such that the results will be dependent on the direction in which the perturbation is performed~\cite{ross2020enhancing}.
...
\begin{itemize}
    ...
    \item Apply the appropriate correction terms (for example for multiple chemical states)
...
\begin{checklist}{How do I know which simulations are unreliable?}
    ...
    \item The ligand is drifts out of the binding site at any $\lambda$ state
        \end{itemize}
\end{checklist}
