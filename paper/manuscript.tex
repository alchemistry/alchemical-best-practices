...
\end{equation}
where $\Omega$ denotes the state space and $f : \Omega \rightarrow \mathbb{R}$ is a sufficiently nice function. Even though not all estimators can be written in the form \eqref{eqn:empirical-average}, it will be sufficient to consider this case to get the main idea. The key object of study in this section is an estimator of 
\begin{equation}
    \mathrm{Var}_{Z_0 \sim \rho_0}\left(\frac{1}{N}\sum_{i=0}^{N-1} f(Z_i)\right)
\end{equation}
where $\rho_0(z)$ is the initial density for $Z_0$.
...
For systems involving a protein, $\rho_t (x | x_0)$ is usually sensitive to $x_0$ and additionally may not exhibit a clear plateau on any computationally feasible time $t > 0$ for "most" $x_0$. In such a case, we can still draw independent samples from $\rho_t (\cdot | x_0)$ by simulating a trajectory up to time $t$ and taking the last frame, but this is much less valuable than knowing that $\rho_t (\cdot | x_0) \approx \rho_T (\cdot | x_0)$ for all $t \in [T_{\min}, T]$ and all $x_0$  in some explicitly definable subset of $\Omega$.\footnote{Should one define the mixing time of a Markov process? Cite Peres Markov Chains and Mixing Times, which is freely available?} In this section, we explore the various possible types of error analysis that can be performed, depending on the behavior of $\rho_t$. 
...
\textbf{Definition}:
We define an \textbf{ensemble} to be a set of $R \geqslant 1$ trajectories, all started at the same initial configuration $x_0$, with initial velocity $v_0$ independently drawn from the Maxwell-Boltzmann distribution at the appropriate temperature and run for a chemical time $T$. We emphasize that, with this definition, all ensemble-based estimates are inherently tied to the choice of $x_0$; pathological choices  -- for example, an unbound protein–ligand configuration -- can render the resulting estimates meaningless.

\subsubsection{Ensemble error estimates}

This section describes the simplest and, in many cases, most robust procedure for estimating uncertainties in free energy calculations. For most practical applications, we recommend this approach when reporting statistical error bars. The method consists of simulating an ensemble $\{X^{(1)},\ldots,X^{(R)}\}$ of $R$ molecular dynamics trajectories initialized at $x_0$, and using the resulting free energy estimates $\{\Delta G^{(1)},\ldots,\Delta G^{(R)}\}$ to quantify uncertainty. The trajectories $\Delta
G^{(i)} (\cdot)$ are identically distributed and conditionally independent given $x_0$. For any time $T > 0$, the variance of any $\Delta G^{(i)} (T)$ can be estimated with the sample variance $S^2_{(R - 1)} (T)$,
...
\end{equation}
which is the gold standard for reporting the error of a free energy estimate $\overline{\Delta G (T)} $. Note that $\mathrm{SE}_R
(T ; x_0) \rightarrow 0$ as $R \rightarrow \infty$ so long as $\mathrm{Var}_{x_0}(\Delta G^{(1)}(T))$ is finite, which is true if the $\Delta G^{(1)}(T)$ are (say) bounded. 
Consequently, we can estimate the variance of the estimate to essentially arbitrary precision. Note, however, that $\Delta G^{(1)}(T)$ will not in general be normally distributed, and the variance may not be a good statistic to capture the error, as the distribution may be multi-modal and heavy-tailed.
...
In principle, the most robust error estimates can be obtained by drawing the initial condition $x_0$ from a suitable initial density $\rho_0(x)$ and computing means and standard errors, but as mentioned in Section \ref{sec:timescales}, the value of this approach is limited by the user's ability to choose $\rho_0(x)$.
...
\]
is finite. While one might hope for the asymptotic scaling $E(t)\sim t^{-1/2}$, such behavior is rarely observed at practically accessible timescales. Nevertheless, conditioned on their initial states, the estimates $\{\Delta G^{(i)}(t)\}_{i=1}^R$ obtained from different replicates are independent and identically distributed. Consequently, the conditional variance of the averaged estimator
\[
\frac{1}{R}\sum_{i=1}^R \Delta G^{(i)}(t)
\]
scales as $E(t)^2/R$, and the corresponding conditional error scales as $E(t)/\sqrt{R}$.

Given a fixed computational budget $B$, we may consider simple allocation schemes in which $B = RT$, where $R$ replicates are each run for time $T$. Under this constraint, the conditional error scales as
\[
\frac{E(T)}{\sqrt{R}}
= \frac{E(T)}{\sqrt{B/T}}
...
\]
The function $J(T)$ thus quantifies the tradeoff between trajectory length and the number of replicates. In practice, we frequently observe that $J(T)$ is minimized at small values of $T$. 
