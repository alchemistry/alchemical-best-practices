...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Tweaks to formatting of title page
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Move the title up
\makeatletter
\renewcommand{\maketitle}{%
  \vskip0pt%  % Reduced from 36pt
...
%% Shrink author and affiliation text
\makeatletter
\renewcommand{\Authfont}{\bfseries\small\raggedright}  % Authors smaller and not bold
\renewcommand{\Affilfont}{\mdseries\small\raggedright}  % Affiliations smaller
\setlength{\affilsep}{8pt}  % Reduce spacing between author and affiliations
\makeatother
...
\affil[20]{Structural Bioinformatics and Computational Biochemistry, Department of Biochemistry, University of Oxford, South Parks Road, Oxford, OX1 3QU, U.K.}
...
\affil[27]{Laboratory for Biomolecular Simulation Research, Institute for Quantitative Biomedicine, and Department of Chemistry and Chemical Biology, Rutgers University, Piscataway New Jersey, 08854}
\affil[28]{Wadhwani School of Data Science and Artificial Intelligence, Indian Institute of Technology Madras, Chennai, Tamil Nadu, India 600 036}
\affil[29]{Department of Pharmaceutical Sciences, Pharmaceutical Chemistry Division, Josef-Holaubek-Platz 2, University of Vienna, 1090 Vienna, Austria}
\affil[30]{Vienna Doctoral School of Pharmaceutical, Nutritional and Sport Sciences (PhaNuSpo), University of Vienna, 1090 Vienna, Austria}
...
\orcid{Michael Caspary}{0000-0001-8081-6373}
...
\orcid{Michael Snarski}{0000-0002-8041-2210}
\orcid{Sara Tkaczyk}{0000-0002-8322-1700}
\orcid{Gary Tresadern}{0000-0002-4801-1644}
\orcid{Huafeng Xu}{0000-0001-5447-0452}
\orcid{Darrin M. York}{0000-0002-9193-7055}
...
%% Expand title, authors, and affilitations sideways to keep title and abstract on first page
...
Alchemical free energy calculations are a useful tools for predicting free energy differences associated with the transfer of molecules from one environment to another.
The hallmark of these methods is the use of ``bridging'' potential energy functions representing \emph{alchemical} intermediate states that do not exist as real chemical species. The data collected from these bridging alchemical thermodynamic states allows the efficient computation of transfer free energies (or differences in transfer free energies) with orders of magnitude less simulation time than simulating the transfer process directly. 
While these methods are highly flexible, significant care must be taken in avoiding common pitfalls to ensure that computed free energy differences can be robust and reproducible for the chosen force field, and that appropriate corrections are included to permit direct comparison with experimental data.

In this paper, we review current best practices for several popular application domains of  alchemical free energy calculations, with a particular focus on the intricacies and challenges of relative and absolute free energy calculations of the binding of small molecules to biomolecular targets. We focus on ``single calculations'' where the quantity of interest is a single free energy difference (or difference of differences); best practices for large-scale free energy calculations are discussed in an upcoming companion article~\cite{mey_largescale_inprep}.
...
\begin{itemize}
\item Computing partition ($\log P$) or distribution ($\log D$) coefficients of a molecule moving between different environments (Sec.~\ref{sec:theory-partitioning})~\cite{rustenburg2016measuring, bosisio2016blinded} 
\item Determining the free energy of partitioning between compartments into membranes~\cite{corey2019insights}. 
\end{itemize}

Furthermore, alchemical calculations are frequently used to estimate changes in free energies upon modifying a ligand or protein. Specifically:
\begin{itemize}
\item A protein residue can be alchemically mutated to probe the impact on binding affinity~\cite{hauser2018predicting,aldeghi2018accurate} or changes in protein thermostability~\cite{seeliger2010protein,gapsys2016insights,gapsys2016accurate,aldeghi2019accurate}; 
\item The entire ligand can be alchemically transferred from protein to solvent in an absolute binding free energy calculation (Sec.~\ref{sec:theory-abfe})~\cite{mobley2007predicting,aldeghi2015accurate,aldeghi2017predictions}; 
\item Small alchemical modifications can be made between chemically related ligands to estimate relative differences in binding free energies (Sec.~\ref{sec:theory-rbfe})~\cite{wang2015accurate,mey2016blinded,song2019using,gapsys2020large,kuhn2020assessment}.
\end{itemize}


After an alchemical calculation is performed, which frequently involves multiple simulations at a variety of alchemical states, the data must be analyzed to compute an estimate of the free energy for the transformation of interest.
Early work used simple but statistically suboptimal estimators for this: free energy perturbation (FEP) uses a simple (but highly biased) estimator based on the Zwanzig relation~\cite{zwanzig1954hightemperature} or numerical quadrature via thermodynamic integration (TI), for which the theory dates back the better part of a century but with the first computational applications emerging in the 1980's and 90's~\cite{kirkwood1935statistical, jorgensen1985monte, kollman1993free, wong1986dynamics, merz1989free}. %
...
This foundational work, combined with the methodological, technological, and hardware improvements of the last 10--15 years, has led to an explosion of interest and direct commercial application of these technologies~\cite{wang2015accurate, fratev2019improved, schindler2020largescale, cournia2017relative, sherborne2016collaborating, kuhn2020assessment}. Macuglia et al. recently reviewed the evolution of biomolecular alchemical free energy calculations into practically useful tools~\cite{macuglia2025dawn}.
...
In part because of the many advances that have been made in developing these important computational approaches, today's users may find it difficult to get started with these complex calculations while also keeping up with the fast pace of change. 
This ``Best Practices'' guide aims to provide current recommendations and tips for users of all experience levels, providing context with other new methods being developed. Updates and suggestions are highly encouraged via our GitHub repository at \url{https://github.com/alchemistry/alchemical-best-practices}.    

\begin{figure*}
    \centering\includegraphics[width=1.0\linewidth]{paper/figures/fig1_what_is_alchemy/calculation_types_and_workflow.png}   
    \caption{\textbf{Illustration of common types of free energies differences that can be calculated using alchemical free energy methods (blue box) and a typical workflow for executing a relative binding free energy calculation (yellow box).}
    \textbf{Blue box}: Labels map transformation types to manuscript subsections. \textbf{Sec.~\ref{sec:theory-solvation}}: Solvation free energy. \textbf{Sec.~\ref{sec:theory-partitioning}}: Partition ($P$) and distribution coefficients ($D$) depend on the change in free energy between different phases; partitioning between water and methanol and water and a membrane are shown as examples. \textbf{Sec.~\ref{sec:theory-abfe}} Absolute free energies of binding of a small molecule to a host (e.g. protein). \textbf{Sec.~\ref{sec:theory-rbfe}} Relative free energy of binding of one molecule with respect to another, here toluene and benzyl alcohol. The same theory applies to the change in binding free energy due to mutation of the host, though we do not discuss these calculations in detail here. Changes in free energy due to a conformational change of the molecule across a high barrier (illustrated here by the torsion reaching from the aromatic ring to the hydroxyl group) can be calculated by alchemical means, though this is not discussed here.
    \textbf{Yellow box}: Before running alchemical calculations, it should be assessed whether the problem is suitable (Sec.~\ref{sec:step0}). \textbf{A}: The protein structure of the system is prepared such that all structural issues are resolved and proper protonation is applied (Sec.~\ref{sec:prerequisites}). All query ligands are positioned into the binding site. Here, the quantity of interest is the relative binding free energies for a set of ligands and a suitable protocol is selected (Sec~\ref{sec:simulation_protocol_choice}) \textbf{B}: Relative binding free energies for $N=6$ compounds are predicted in this example, and $n=7$ transformations (edges) are selected (see~\cite{mey_largescale_inprep} for a discussion of selecting transformations). \textbf{C}: for each transformation, a $\lambda$ decoupling parameter is applied to guide the transformation. \textbf{D}: for each transformation and $\lambda$ window a simulation is run. \textbf{E}: for each transformation, the relative binding free energy ($\Delta\Delta$G$_{bind}$) is calculated given $\lambda$. \textbf{F}: A binding free energy difference is predicted for a transformation (edge) (Sec~\ref{sec:data_analysis}). \textbf{G}: a final calculation is done to estimate the absolute binding free energy of each compound using for example a maximum-likelihood estimator (see~\cite{mey_largescale_inprep}). \textbf{H}: Absolute binding free energy estimates are ready to be used for prioritization in synthesis. Note that here focus on ``single calculations'' where the quantity of interest is a single free energy difference (or difference of differences); best practices for large-scale free energy calculations are discussed in a companion article~\cite{mey_largescale_inprep}.\label{fig:fig_what_is_alchemy}
    }
\end{figure*}
...
This ``Best Practices'' guide is intended as both a good starting point for new practitioners and a reference for experienced practitioners. 
...
We also assume a basic familiarity with the principles of molecular mechanics, molecular dynamics simulations, statistical mechanics, and the biophysics of protein-ligand association. If you feel unfamiliar with some of these concepts, good starting points can be found in the followingreferences: ~\cite{braun2019best, grossfield2018best, klimovich2015guidelines, shirts2012best}. 
...
\begin{itemize}
    \item Is my problem suitable for an alchemical calculation? (Sec.~\ref{subsec:suitability}) 
     \item How do I prepare my system to carry out an alchemical calculation? (Sec.~\ref{subsec:binding})
     \item How do I select an appropriate simulation protocol? (Sec.~\ref{sec:simulation_protocol_choice})
     \item What software tools are available to perform alchemical calculations? (Sec.~\ref{sec:software}) 
     \item How should I analyze my data and report uncertainties? (Sec.~\ref{sec:data_analysis})
     \item What are the unsettled practices and current limitations of alchemical methods? (Sec.~\ref{sec:unsettled})
\end{itemize}

Notably, this document---version 2.0 of the original guide---focuses specifically on best practices on ``single'' alchemical free energy calculations where the quantity of interest is a single free energy difference (or difference of differences). A companion guide, \textit{Best Practices for Multiple Alchemical Free Energy Calculations}~\cite{mey_largescale_inprep}, covers the considerations involved in high-throughput or large-scale campaigns, such as automation, perturbation network design, and campaign-level data management and analysis. Readers aiming to scale up to multiple calculations will find many important suggestions and ideas relevant to those problems in that document.

In this guide, some additional background information may be needed depending on the nature of the alchemical project. For example, if binding poses are not known, docking calculations or co-folding models can be used to generate an initial small molecule binding pose to start alchemical simulations\cite{scheen2025leveraging, thaler2026boltz, coskun_2023, beuming_2022}. This will require some basic familiarity with how to perform docking or use generative models to generate reasonable simulation starting points~\cite{grinter2014challenges}. 
As some of the theoretical background can be daunting to beginners, we do, however, provide a guide to the essential theory behind alchemical free energy calculations in Sec.~\ref{sec:theory}.

In the remainder of this paper, we will cover topics that are key to the preparation of the simulation (Sec.~\ref{sec:prerequisites}), the choice and use of correct simulation protocols (Sec.~\ref{sec:simulation_protocol_choice}), and finally the best practices that should be used in the analysis of alchemical calculations (Sec.~\ref{sec:data_analysis}). 
We will give particular focus to aspects of the molecular simulations which are unique to alchemical calculations---in particular, the calculation of transfer free energies (hydration free energies, partition coefficients, etc.) and binding free energies (absolute and relative). 
...
\item Approximate and usually less accurate endpoint free energy methods such as MM-PBSA~\cite{genheden2015mm} and LIE~\cite{gutierrez-de-teran2012linear}
...
\item Forcefield dependence for protein, ligand, ions, co-solvents, and co-factors. A number of different studies have looked at the influence of force fields and it is assumed the user has made an appropriate choice of force field for the system under study~\cite{loeffler2018reproducibility, vassetti2019assessment, lopes2015current, hahn2024current}. 
...
$C^{\circ}$, $C_u$ --- standard and actual molar solute concentrations \\
...
$L$ and $R$ --- generic labels for ligand and receptor\\
$\mathcal{O}$ --- overlap matrix between alchemical states\\
$P^{\circ}$ --- standard pressure \\
$p$ --- pressure \\
$\vec{q}$ --- vector of a single configuration, i.e. $x$, $y$, $z$ coordinates of the simulation system\\
$q'_u$ --- molecular partition function of the solute at rest \\
$T$ --- temperature \\
...
$x_u$ --- internal coordinates of the solute \\
$z_u$ --- intramolecular configurational partition function of the solute \\
$Z$ --- partition function \\
$\beta$ --- inverse thermal energy $(k_B T)^{-1}$ \\
...
$\lambda$ --- alchemical progress parameter, which may be multidimensional if there are multiple dimensions along which the system is changing \\
$\lambda_i$, $\Lambda_u$ --- atomic and molecular thermal de Broglie wavelengths \\
$\mu^\circ$, $\mu$ --- standard and actual solute chemical potential \\
$\Psi_u$ --- potential of mean force of the solute \\
$\tau _{eq}$ --- integrated auto-correlation time\\
$\zeta$ --- external coordinates of the solute
\end{tcolorbox}
...
In this section, we use the statistical mechanics theory of dilute solutions to derive free energy expressions that form the basis of alchemical computational protocols for modeling noncovalent molecular binding, solvation, and solvent partitioning equilibria. The emphasis here is placed on bridging theoretical foundations and intuition. Since chemical equilibria are governed by the chemical potentials of the species involved, we begin by considering a statistical mechanics expression for the chemical potential of a solute in a solvent. We will then combine chemical potentials to develop models for each process.
...
\end{equation}
where $k_B$ is Boltzmann's constant, $T$ is the absolute temperature, $C_u = N_u/V$ is the number concentration of the solute, $\Lambda_u = h/\sqrt{2 \pi M_u k_B T}$ is the thermal de Broglie wavelength of the solute, where $h$ is Planck's constant and $M_u$ is the solute's total mass. 
An ideally dilute solution is a potentially non-physically realizable solution in which solute molecules interact only with the solvent, and solute-solute interactions are negligible.~\cite{levine2009physicalchemistrybook6ed}

The quantity $q'_u$ in Eq.~(\ref{eq:chemical-potential-definition}) is the molecular partition function of the solute at rest in the solvent
\begin{equation}
  q'_u = \frac{\Lambda_u^3}{\prod_i \lambda_i^3} 8 \pi^2 z_u
  \label{eq:intra-qpu-def}
\end{equation}
where $\beta = (k_B T)^{-1}$, $\lambda_i$ is the thermal De Broglie wavelength of the $i$-th atom of the solute (not the alchemical parameter as used elsewhere in this document), and $z_u$ is the intramolecular configurational partition function of the solute
\begin{equation}
...
A few explanatory notes are in order. First, Eq.~(\ref{eq:chemical-potential-definition}) is derived using classical statistical mechanics theory~\cite{hill1986statthermo}. Classical mechanics is considered an excellent approximation for the room temperature processes studied here, which involve only changes in intermolecular interactions. A quantum-mechanical treatment is necessary for processes not covered here, such as covalent binding, that involve breaking and forming chemical bonds. Classically, equilibrium thermodynamic properties, such as gas solubilities and binding constants, do not depend on atomic masses. Hence, while they are included in Eqs.~\ref{eq:chemical-potential-definition} and Eq.~\ref{eq:intra-qpu-def} for completeness, the thermal De Broglie wavelength terms coming from the partition functions of the momenta cancel in the resulting expressions below, and we will no longer consider them.

Eq.~(\ref{eq:chemical-potential-definition}) is derived by writing the classical canonical configurational partition function $Z(N_u = 1, N_v)$ of one solute molecule in a solvent of $N_v$ molecules in a volume $V$ and multiplying and dividing it by configurational partition function of the pure solvent [Eq.~(\ref{eq:Z-pure-solvent})], and then separating out the integration over the external degrees of freedom of the solute (translations and rotations) giving an $8 \pi^2 V$ term to yield $Z(N_u = 1, N_v) = Z_{N_v} 8 \pi^2 V z_u$ where $z_u$ is defined by Eqs.~(\ref{eq:intra-zu-def}) and (\ref{eq:solvent-pmf-def}). The canonical configurational partition function of $N_u$ independent solute molecules in a solvent is then obtained by considering the product of the solute terms $Z(N_u, N_v) = Z_{N_v} (8 \pi^2 V z_u)^{N_u}/N_u!$,~\cite{guggenheim1952mixtures504,simonson2016physical} where the factorial term accounts for indistinguishability. Finally, the canonical partition function $Q(N_u, N_v)$ of the solution is obtained by including the appropriate terms coming from the momenta and differentiating the resulting Helmholtz free energy $A(N_u, N_v) = -k_B T \ln Q(N_u, Nv)$ with respect to $N_u$ to obtain Eq.~(\ref{eq:chemical-potential-definition}).

The form Eq.~(\ref{eq:chemical-potential-definition}) for the solute chemical potential using the potential of mean force description is useful, as we will see, because it separates the solute's translational motion, which leads to the concentration dependence, from the intramolecular contributions. The potential of mean force description does not introduce approximations. With the exception of the use of classical statistical mechanics and the assumption  that solute-solute interactions are negligible, Eq.~(\ref{eq:chemical-potential-definition}) is rigorous. Due to rotation-vibration couplings, the separation of the solute's overall rotational motion leading to the $8 \pi^2$ term in Eq.~(\ref{eq:intra-qpu-def}) is not exact. While it is not a strict theoretical requirement, the separation of rotational degrees of freedom is adopted here because it is considered an excellent approximation at room temperature and a useful simplification.

We derived Eq.~(\ref{eq:chemical-potential-definition}) using the canonical (constant volume) partition function of the solution. Since the chemical potential can be equivalently obtained from the constant-volume differentiation of the Helmholtz free energy or the constant-pressure differentiation of the Gibbs free energy, Eq.~(\ref{eq:chemical-potential-definition}) is correct. However, the volume dependence of the chemical potential has to be considered when modeling the Gibbs free energy change for constant-pressure processes where the system's volume changes significantly~\cite{gilson1997statisticalthermodynamic}. Terms arising from volume variations at constant pressure are generally considered small for solvation, solvent partitioning, and molecular binding processes at atmospheric pressure and are often ignored in alchemical calculations.  This is because when a single molecule is removed, errors scale with the fractional change in the system size.  When one particle in 1000 disappears, this will be negligible with respect to other sources of error. Hence, we will use Eq.~(\ref{eq:chemical-potential-definition}) to derive expressions for $\Delta G$'s.

The standard chemical potential of a solute in an ideally dilute solution is obtained from Eq.~(\ref{eq:chemical-potential-definition}) by setting the concentration to the standard concentration $C^\circ$, generally $1$ mol/L equivalent to $1/1661$ particles/\AA$^{3}$, 
\begin{equation}
  \mu_u^\circ = -k_B T \ln \frac{q'_u}{C^\circ \Lambda_u^3} \, .
  \label{eq:chemical-potential-standard}
\end{equation}
The standard state of a solution is defined as an ideally dilute solution at concentration $C^\circ$.~\cite{levine2009physicalchemistrybook6ed} Hence, Eq.~(\ref{eq:chemical-potential-standard}) is formally correct for real solutions.

Next, we outline the application of the chemical potential statistical mechanics formalism to model key biophysical processes: solvation, solvent partitioning, and noncovalent molecular binding. In general, translating statistical-mechanics identities into computable quantities via molecular simulations involves expressing ratios of partition functions in terms of ensemble averages. In the present context, the quantity being averaged often turns out to be the exponential of a perturbation energy function (the \textit{Zwanzig relationship}~\cite{zwanzig1954hightemperature}), which is efficiently evaluated using the stratification algorithms and free energy estimators discussed in Sections \ref{subsec:formulations-abfe-rbfe}, \ref{subsec:numerical-implementations},  and \ref{subsec:estimators} rather than directly. In this section, we assume that an implementation is well-formed when expressed as a combination of exponential averages, assuming they can be evaluated in practice. As in the previous section, we use the solvent potential of mean force to simplify the notation when possible. The solvent degrees of freedom can be restored in the resulting expressions using Eq.~(\ref{eq:solvent-pmf-def}). 

\subsection{Solvation free energy}\label{sec:theory-solvation}
...
\end{equation}
where we use Eq.~(\ref{eq:chemical-potential-standard}) for the solution, Eq.~(\ref{eq:chemical-potential-definition}) for the gas (assumed ideal) at the standard gas concentration $C = P^\circ/k_B T$ where $P^\circ = 1$ bar is the standard pressure, and Eq.~(\ref{eq:intra-qpu-def}) noting that all momentum terms cancel. The first term in Eq.~(\ref{eq:DG0-solvation-def}) is an ideal term $\Delta G^\circ_{\mathrm{slv,ideal}}$ that accounts for the difference of standard states for solution and gases. The second term is the excess solvation free energy $\Delta G_{\mathrm{slv, exc.}}$ that can be evaluated by numerical alchemical calculations as described in this document. Hence Eq.~(\ref{eq:chemical-potential-standard}) can be used to compare calculated and experimental standard free energies of solvation\cite{thompson2004new}.

However, equilibrium experimental gas solvation data are most often reported in terms of Henry's constants $k_H$. Setting $\Delta G_{\mathrm{slv}} = 0$ when the gas partial pressure and the solute's concentrations are equal to their equilibrium values $P_A$ and $C_A$, respectively, we obtain
\begin{equation}
  P_A = k_H C_A
  \label{eq:Henrys-law}
\end{equation}
...
\end{equation}
The relation above is used to connect alchemical calculations to Henry's constant values, generally after converting them to the excess free energy scale~\cite{cabani1981group,nicholls2008predicting,mobley2014freesolv}
\begin{equation}
\Delta G_{\mathrm{slv, exc.}} = - k_B T \ln \frac{z_{A(v)}}{z_{A(\mathrm{g})}} = k_B T \ln \frac{k_H}{k_B T} \, .
\label{eq:DG-solvation-excess}
\end{equation}
Eq.~(\ref{eq:DG-solvation-excess}), which involves only intramolecular partition functions (as described above),  states that the excess free energy of solvation corresponds to the process of moving the solute from a fixed position and orientation in the gas to a fixed position and orientation in the solvent. This is sometimes referred to as the solvation free energy in the Ben-Naim standard state~\cite{bennaim1984solvation}.
...
\end{equation}
The objective of alchemical solvation free energy calculations is to implement Eq.~(\ref{eq:zratio-solv-dgexc}), by means of \emph{coupling} or \emph{decoupling} computational protocols and alchemical intermediate states as discussed in section \ref{subsec:numerical-implementations}.  Subsequent sections will also discuss computationally efficient approaches to calculate this ratio of integrals (Sec.~\ref{subsec:estimators}).

\subsection{Free energy of solvent partitioning}\label{sec:theory-partitioning}
...
immediately identifies the partition coefficient with the ratio of intramolecular configurational partition functions of solute $A$ in the two solvents:
\begin{equation}
...
\end{equation}
is the statistical mechanics expression for the binding constant. Eq.~(\ref{eq:DG0-binding-constant-def}) follows from the application of  Eq.~(\ref{eq:chemical-potential-standard}) for each of the three species and noting that the contributions of the momenta cancel. As discussed below, various alchemical protocols exist to evaluate the ratios of partition functions in Eq.~(\ref{eq:DG0-binding-constant-def}), thereby connecting the theory to observed binding affinities.
...
The definition of the bound macrostate of the complex is an essential input of the theory. Unlike bond lengths, bond angles, and dihedral angles, the three degrees of freedom of the complex that correspond to the position of the ligand relative to the receptor are unbounded. Unless their extent is limited in some fashion, the bound state of the complex cannot be distinguished from the unbound state, and their free energy difference will be undefined. For example, it is reasonable to insist that the ligand be close to the receptor in configurations regarded as bound. The definition of target macrostates is required in many areas of computational chemistry and biophysics, for example, when specifying the range of backbone angles to measure the relative populations of $\alpha$-helical and $\beta$-strand conformations of peptides.  

Conversely, the value of the binding free energy depends on the chosen structural definition of the bound complex. The relationship between the predictions of this theory and experimental reporters (thermochemical, spectroscopic, inhibition kinetics, and others) of complex formation has been discussed.~\cite{gilson1997statisticalthermodynamic,mihailescu2004theory,gallicchio2011recent,simonson2016physical,gallicchio2021comppeptsci} The interpretation of experimental reporters of binding is a complex issue in itself. Still, the consensus is that this theory yields nearly equivalent results when binding is strong and specific, and the definition of the bound complex encompasses the major bound poses while excluding obvious unbound configurations. In some cases, it is valuable to consider specific definitions of the complex, for example, by restricting the ligand's orientation to pinpoint the relative contributions of different binding poses.

Eq.~(\ref{eq:DG0-binding-constant-def}) is the basis of a variety of alchemical computational implementations that have been developed to study molecular binding by alchemical means. Some of these are discussed in Section \ref{subsec:formulations-abfe-rbfe}. Generally, to formulate the statistical mechanics expression of the binding constant Eq.~(\ref{eq:DG0-binding-constant-def}) as an ensemble average, we first need to ensure that the configurational integrals in the numerator and denominator are over the same set of degrees of freedom. Note that the intramolecular configurational partition function of the complex, $z_{RL(v)}$, includes six more degrees of freedom---the position and orientation of the ligand relative to the receptor collectively described by the variable $\zeta$---than the product of the intramolecular partition functions, $z_{R(v)} z_{L(v)}$, of the separated receptor and ligand. To correct this discrepancy, we multiply and divide the right-hand side of Eq.~(\ref{eq:DG0-binding-constant-def})  by the quantity
...
\label{sec:theory-abfe}

In the alchemical literature, the term ``absolute binding free energy'' is another name for the standard binding free energy $\Delta G^\circ_b = - k_B T \ln K_b$ between a receptor and its ligand.  Here, we discuss two methods, Double-Decoupling (DDM) and Alchemical Transfer (ATM), used to evaluate it by alchemical computer simulations.
...
The RBFE alchemical transfer formulation follows the ABFE formulation closely. Similar to the dual coordinate method/approach, the simulation system includes both ligands, one in solution and the other in the binding site, and the coordinate transformation involves displacing them in opposite directions so that they switch places (Figure \ref{fig:atm-illustration}B). Formally,~\cite{azimi2022relative} the method derives from Eq.~(\ref{eq:DG0-binding-constant-def}) for each binding constant
\begin{equation}
...
Roughly, the phase-space overlap between two thermodynamic states measures the degree to which high-probability configurations (i.e., those with low potential energy) in one state are also high-probability configurations in the another state (see Sec.~\ref{sec:are-they-good} and Fig.~\ref{fig:fig_what_is_lambda}).

Equilibrium free energy calculations, solve the problem of having poor overlap between the states of interest by introducing multiple intermediate alchemical states at values $\vec{\lambda}_A = \vec{\lambda}_0, \vec{\lambda}_1, \cdots, \vec{\lambda}_K = \vec{\lambda}_B$ so that each pair of consecutive states $\vec{\lambda}_k, \vec{\lambda}_{k+1}$ share good phase space overlap, meaning, as described above, that there  are decent number of samples that would have moderate probability in both neighboring alchemical states. We make "good overlap" more quantitative in section~\ref{sec:data_analysis}. 
Each intermediate state models a ligand that is neither $A$ nor $B$ but an interpolation of the two.
...
\paragraph{How do absolute free energy calculations differ from relative free energy calculations?}
...
Alchemical free energy calculations involving small molecules seem to achieve, for common benchmark systems, root mean square (RMS) errors relative to experiment around 1-2 kcal/mol depending on force field, system, and a variety of other factors such as simulation time, sampling method, and whether the calculations employed are absolute or relative~\cite{baumann2025largescale, fuMetaAnalysisRevealsThat2022}. Example datasets and case studies are available in the companion article on best practices for large-scale alchemical free energy calculations~\cite{mey_largescale_inprep}.
However, the domain of applicability is a significant concern~\cite{sherborne2016collaborating, cournia2017relative, baumann2025largescale}, especially for protein-ligand relative free energy calculations, which typically require a high quality and usually experimental bound structure of a closely
related ligand as a starting point. Additional factors such as slow protein or ligand rearrangements, uncertainties in ligand binding mode, or charged ligands can make these calculations far less reliable and more of a research effort. A recent study from the Open Free Energy project showed that the accuracy obtained across a set of private industrial drug binding targets (weighted RMSE 1.7 [1.5, 2.0] kcal mol$^{-1}$) was substantially larger than that obtained for common benchmark protein-ligand systems (weighted RMSE 2.4 [1.9, 3.1] kcal mol$^{-1}$), highlighting the increased challenge of real-world application of these methods~\cite{baumann2025largescale}.
...
Simulations carried out with molecular dynamics or Monte Carlo techniques are fundamentally stochastic calculations, and two simulations can never match exactly~\cite{caves1998locally, coveney2016, mdbook2025}.  Even though Newton's equations of motion are deterministic, molecular systems are chaotic, and very small changes, even as small as the  differences in rounding of floating point numbers due to different software versions or architectures, let alone different random number seeds used to initialize the velocities, will lead to very different results obtained with each repetition of a simulation. Any sort of agreement will only be up to this stochastic source of uncertainty, and any researcher needs to understand before starting to perform any calculations how little statistical noise a calculation is needed to draw a scientific conclusion. This also means that proper handling of statistics to decide if differences in simulations are statistically significant or not.  
There is growing literature describing the theoretical underpinning behind such intrinsic non-reproducibility of MD simulation based predictions. Coveney \textit{et al.}~\cite{coveney2016} have noted that, for a  dynamical system to reach equilibrium, the dynamics must at least be ``mixing'' which is an established result in ergodic theory. A system which is ``mixing'' is also ``ergodic'' by definition but the reverse is not true. This implies that MD trajectories are chaotic, and hence one-off simulations can never be reproducible. Thus, it is a necessity to perform ensemble simulations in order to perform ensemble averaging to obtain thermodynamic quantities in a reliable manner. Coveney and Wan recently published a book on the probabilistic foundations of MD and how this approach naturally allows for the treatment of uncertainties that arise in the use of the methodology~\cite{mdbook2025}. In practice, in many cases the simplest and most effective way of performing ensemble simulations is to vary only the initial velocities as these yield reproducible (i.e. statistically robust) free energy predictions. 
Because free energy differences are calculated from ensemble averages, collecting more data will result in improved agreement in almost all cases, thanks to the central limit theorem, but the amount of simulation needed to converge to a desired level of statistical precision varies dramatically on the system. So the key consideration for using free energy calculations is how reproducible alchemical free energy calculations are in practice, and how large a difference between two measurements is required to be scientifically useful.

There are two types of reproducibility that are important to examine.  One is reproducibility between different research groups attempting to reproduce the same results. This situation is most relevant when new users might be trying to see how well their calculations match with published calculations. The second situation reproducibility when one researcher runs the same calculation multiple times, differing only in starting configurations.  This can be particularly relevant in determining the statistical uncertainty of the calculation, as discussed in Sec~\ref{subsec:uncertainty}.
...
The differences in free energies between different research groups, due to differences in choices of simulation parameters, simulation packages, and analysis treatments can be significant. Issues of reproducibility of free energy calculations across different simulation packages have recently attracted attention ~\cite{loeffler2018reproducibility,rizzi2019sampl6, wade2022, herz2025modular}. Greater variability is expected between simulations carried out between packages, as opposed to ones carried out with the same package, due to methodological differences such as integrators, thermostats, barostats, treatment of long-range electrostatics, and potentially other factors. For absolute and relative hydration free energies of small organic molecules a variability of approximately 0.2 kcal/mol between popular simulation packages has been reported~\cite{loeffler2018reproducibility}. In the recent SAMPL6 SAMPLing challenge a larger variability of 0.3 to 1.0 kcal/mol was noted in the computed absolute binding free energies of host/guest systems, even though the study sought to use identical input and simulation parameters~\cite{rizzi2019sampl6}. This variability in free energy calculations occurred even when the energies of individual frames were identical or nearly so.  For more complex use cases, such as protein-ligand binding free energies, the repeatability is often substantially worse~\cite{rizzi2019sampl6, wade2022}. 
However, a recent study demonstrates reproducibility across software packages as well as free energy estimators when ensemble simulations are employed~\cite{wade2022}. The authors reported results from two different MD engines (NAMD and OpenMM) as well as two free energy estimators (FEP and MBAR) for a set of 54 protein-ligand complexes which were found to be in good agreement when ensemble simulations were used to control aleatoric uncertainties. Further work is needed to ensure reproducibility across software packages arising due to systematic/parametric uncertainties. 
For benchmark simulations of some relative binding free energies, differences of {\color{red}Can we put some more citations and discussions of studies here?} 

A different situation is the amount of reproducibility researchers should expect when performing repeated simulations of their own systems, with either the same or different starting points. In simple cases, such as absolute hydration free energies of small organic molecules, or relative hydration free energy calculations between structurally similar small organic molecules, it should be possible to obtain highly precise estimates with a given software package, i.e., with a sample standard deviation under 0.01 kcal/mol possible~\cite{rizzi2019sampl6}, and agreement under 0.05 kcal/mol routine for simulations lasting 10's of ns~\cite{paliwal2011benchmark} {\color{red}This could use some more citations / information here from other studies - I'm just using my examples} However, for complex systems, the reproducibility can be signficantly worse, as two simulation of the same system can explore very different configurational ensembles~\cite{bhati2017, bhati2018, bhati2019, bhati2022, bhati2023, bhati2025}.  If the timescales of the slow scales of motion are comparable to or longer than the timescales of the simulation, then differences of up to multiple kcal/mol can be possible for absolute and even relative free energy calculations. To alleviate such issues and make free energy predictions robust, Bhati et al. developed the TIES method~\cite{bhati2017, bhati2022, bhati2025} which is based on the use of ensemble methods, where the ensemble size can be adjusted based on available computational resource and required precision~\cite{wan2023}. Further, this method is extremely flexible and adaptive. For example, one can simply add additional ensemble members as necessary or desired, without having to re-run anything done initially. It should also be noted that ensemble simulations are computationally more effective than long simulations. On a large supercomputer, all the sampling can be done concurrently obtaining the statistical results in the time it takes to run a single simulation. Studies have shown that, in general, an ensemble size of 5 and 10 members is a reasonable starting point for alchemical RBFE and ABFE calculations, respectively, but recommendations are provided to adjust them based on some rules of thumb~\cite{bhati2022, bhati2025}.

In fact, the amount of reproducibility between repeated calculations is an important measure of whether a particular choice of simulation methods and approaches are appropriate.  Large differences between results carried out with different initial velocities, or better yet, started from points along a single initial long simulations can indicate that the sampling of the system is particularly hard and will require additional simulation and/or advances sampling methods.  In fact, journals such as the \textit{Journal of Chemical Information and Modeling require} and \textit{Communications Biology} that any results be confirmed by simulations run in at least triplicate~\cite{soares2023guidelines, wan2023}. The question of how to combine results from ensembles of simulations for lowest bias and best reporting of statistical uncertainty are discussed in section~\ref{subsec:uncertainty}.
...
%MD trajectories are extremely sensitive to their starting conditions and hence single simulationscan never be reproducible~\cite{coveney2016, MDbook2025, caves1998locally}. Thus, it is recommended to perform ensemble simulations in order to perform ensemble averaging to obtain thermodynamic quantities in a reliable manner.
...
\emph{more} challenging than simply measuring them experimentally! This is
...
appeal when (slow or costly) synthesis would be required or experiments are otherwise cost-prohibitive or unavailable.
...
A critical aspect of these calculations is the preparation of the system ahead of simulation.  A careful choice of force field for both the solute and solvent is essential.  Accurate modeling of the solute's solvation behavior hinges on realistic representations of the solvent. See, for example,~\cite{bosisio2016blinded,rustenburg2016measuring} for a good discussion of these force fields choices.  Care should be taken when choosing the forcefield as solvent model parameters may vary substantially between force fields~\cite{jambeck2003, kadaoluwa_2021, mobley_2007charge} and impact results accordingly, and applies to both water and non-aqueous organic solvents. For example, in a study by the Open Force Field Initiative, the  GAFF\cite{wang2004gaff} and Open Force Field Sage~\cite{boothroyd2023sage} force fields produced different results in non-aqueous solutions. 

\paragraph{Equilibration of the Solvent}
Before computing solvation or partition free energies, it is important to ensure that the solvent itself is adequately equilibrated. While water typically equilibrates rapidly (within several hundred ps), especially when prepared using standard protocols, some organic solvents—particularly those with long hydrocarbon chains—can require substantially longer equilibration times~\cite{fan2021sampl7} (10's of nanoseconds). Failing to properly equilibrate the solvent can introduce systematic errors in free energy calculations, as the solvent structure and density may not reflect the true thermodynamic ensemble of the target environment~\cite{zhang2017solventmodel}. This includes equilibrating the system at the pressure of interest, as the initial box may not be the correct size for a given number of particles to result in the appropriate pressure for that number of particles. 

\paragraph{Validation through experimental comparisons}  
Experimental comparisons in solvation systems are often easier and more direct than binding free energies. Hydration and partition coefficients are frequently measured with high precision, enabling rigorous validation of computational protocols. This has led to their prominent use in blind challenges such as SAMPL~\cite{skillman2012sampl3, mobley2014sampl4, bergazin2021sampl7}, where participants predict hydration or partition free energies of small molecules using a variety of force fields, solvent models, and sampling strategies. However, such exercises have repeatedly shown that small changes in simulation parameters or protocols can result in significant deviations from experimental results, reinforcing the need for careful setup and validation even in seemingly ``simple'' solvation systems.  Calculating solvation free energies for charged solutes, however, remains particularly challenging, as long-range electrostatics, finite-size artifacts, and treatment of counterions can significantly affect results. In fact, direct experimental determination of ionic hydration free energies is generally not feasible; instead, such values are inferred by decomposing the solvation free energy of ion pairs~\cite{mobley2014freesolv}. 

\subsection{Free energies of binding}
\label{subsec:binding}
All of the above considerations in Section \ref{subsec:hydration} on preparing solvation free energy and partition coefficient calculations also apply to binding free energy calculations, with added complications that protein-ligand binding systems can have significantly slower configuratonal sampling. In principle, in the limit of sufficient configurational sampling, the free energy changes estimated from an alchemical free energy calculation should be independent of the system's initial coordinates. However, in practice, because simulations are of finite duration (typically 1--100 ns per state at present), this is only true for certain classes of alchemical free energy calculations such as relative or absolute free energies of hydration of small and relatively rigid organic molecules. Protein-ligand complexes often exhibit slowly relaxing degrees of freedom that can significantly exceed the duration of an alchemical free energy calculation, and even smaller host-guest calculations can be susceptible to these issues as well, depending on timescale and system. It is therefore generally important to carefully select input coordinates to maximize the chance of obtaining good results, as described in more detail below.  
...
\paragraph{Are there good starting structures available?}
As with any simulation, care should be taken in selecting available starting structures (X-ray, Cryo-EM, etc) in the Protein DataBank~\cite{berman2003announcing}. In some cases it may be wise to choose multiple starting structures to account for variability in receptor conformations as well as the accuracy of available starting structures. Typically, clustering of receptor structures can be used to identify different receptor conformations near the binding site, as well as assessing relevant side chain placements from the starting structure, see for example~\cite{mey2016blinded}. 

Cofolding models such as AlphaFold-3,~\cite{abramson_2024} OpenFold-3 and Boltz-2~\cite{passaro_2025} can generate starting structures for systems where there may not be exact datasets available in the PDB. Care must be taken to ensure that predicted structures are accurate, especially when it is clear that the predicted system was not present in the cofolding model's training set; conservation of stereochemistry, water placement and other chemical requirements that these types of models sometimes perform poorly on must be paid close attention to. Although no literature exists yet to show this, cofolding models may provide a (partial) method to circumvent error-prone protein preparation by generating structures free of typical structural biology issues such as missing loops/residues. More practically, cofolding models can be finetuned on internal structural data to provide a streamlined approach to system generation while benefiting from some of the models' advantages.~\cite{scheen_2025,thaler2026boltz, beuming_2022, coskun_2023} In terms of set up and other choices, following general best practice guidelines is advisable~\cite{braun2019best}. To ensure modeling matches experiment, we also need to accurately prepare and simulate the same system -- which requires understanding what protein construct is used in the bioassay. For instance, does the X-ray structure that is to be used for the calculations match the construct used for screening (i.e. only the catalytic domain vs. full length, monomer vs. dimer, etc.)~\cite{perez-benito2018predicting}? Also, were certain co-factors or partner proteins required in the bioassay? 

Many free energy calculations focus on a congeneric series of ligands, which can make these calculations suitable for relative free energy protocols (see Sec.~\ref{sec:simulation_protocol_choice}). For relative calculations, some care has to be taken selecting binding poses for these ligands. Generally, a common assumption for a congeneric series is that the binding mode is conserved. Therefore, if a starting structure of one of the ligands is available, this should be used to position the ligands in the putative binding site in an energetically reasonable conformation without steric or electrostatic mismatch with the receptor. Checking the starting structure versus the experimental electron densities is important, as the position of part of the ligand or important sidechains may be based on the interpretation of the crystallographer rather than the available electron density, especially in cases of missing density. For example, looking at a cyclohexane ring density, a chair configuration is vastly more likely than that of a boat and, if a boat configuration is present in the structure, it may be worth inspecting the density to ensure it adequately supports this choice. 

\paragraph{Are you prepared to deal with any binding mode challenges?}
Generally, binding modes within congeneric series are conserved~\cite{wacker2010conserved}, however, exceptions exist~\cite{brandt2011congeneric,nazare2005probing}, as discussed in more detail in Sec.~\ref{sec:multiple_binding_modes}. Certain functional groups may be particularly prone to this due to symmetries or near symmetries. One such issue involves a $180^{\circ}$ flip in the dihedral angle of an aromatic ring, or five-membered ring leading to a different spatial position of ortho- or meta- substituents that otherwise should overlap within a series. The $180^{\circ}$ flip of the ring may not occur sufficiently frequently during simulations due to steric obstructions, meaning the  starting configuration can bias the outcome. Another scenario may arise from equatorial and axially substituted saturated rings (e.g. cyclohexane derivatives). This situation may be addressed by explicitly modeling different binding modes of the same ligand and combining later computed free energy differences for different binding modes into a relative free energies of binding~\cite{kaus2015how}.

\paragraph{Have you considered stereoisomers and enantiomers?}
Congeneric series can contain stereoisomers or enantiomers, which can bind very differently, resulting in large errors if treated incorrectly. For racemates, the relative abundance of each stereoisomer is normally not known; however, when there is only one stereocenter in the molecule it can often be fair to assume that the substance is a 50\%:50\% mixture of the two unless other information is given. If one enantiomer is active (eutomer) and the other is inactive (distomer), this corresponds to the effective concentration of your bioactive molecule being half of what is expected in the assay, which corresponds to a ~0.2 pIC50 loss in potency relative to the pure eutomer being tested. This effect could be captured within analytic calculations.~\cite{h2011significance} If possible, both enantiomers could be considered computationally. However if a series contains multiple stereocenters, or multiple compounds within the series have stereocenters, this explicit enumeration of chiral isomers may be computationally prohibitive. Therefore, the experimental activity associated with just one stereoisomer/enantiomer is more uncertain. If enumerating unknown stereocenters is not tractable, modeling using just the conformation that best fits the active site is reasonable.  This can, of course introduce the potential for larger errors compared to experiment. Additionally, there can unexpected changes in which stereoisomer binds experimentally as a function of R groups studied, making it difficult to compare relative binding free energies along a series. We can see that care and further testing is needed in this scenario, and the quality of the predictions may suffer without such care. 

\paragraph{Are there cofactors present?}
Crystallographic structures can contain cofactors, such as ions, which may be close to the ligand series of interest. Understanding if they are required or not for simulation can be an interdisciplinary effort between the modeller, crystallographer and bioligists to establish if they are a crystallographic artifact, or if they are likely to be present in the assay. In addition to both the crystallographic and assay conditions, what is most relevant \textit{in vivo} might be a third, equally difficult consideration. If a cofactor is included in the calculations, it is worth considering the appropriate force field for the cofactors. Cofactors, in particular metal ions can be challenging\cite{mobley2017predicting, guven2024protocols} because of tight binding to the ligand.  If the congeneric series of ligands is expected to interact with the cofactors differently, such as requiring metal ions in different charge states, or if some molecules in the series displace, then this would require significant extra analysis and bookkeeping. If there is still uncertainty around cofactor binding, running free energy calculations for a set of molecules both with and without the cofactor and comparing the relative rank-ordering of molecules could provide a sanity check.
 

\paragraph{Conserved binding site waters can play an important role in binding free energies}
Binding site water molecules may form water mediated protein-ligand interactions which can pose challenges whenever exchange with bulk water is slow compared to simulation timescales. In particular, this can happen in buried binding sites~\cite{laage2017water}. Overlaying multiple protein X-ray structures can identify conserved water molecules or those that are associated only with certain ligands in a series and are therefore important to include in calculations. In cases where water molecules are known to play an important role in ligand binding, for example if the presence of electron density suggests they are tightly bound (indicated by a high EDIA score~\cite{nittinger2015evidence,meyder2017estimating}), software implementations that use water sampling facilitated by Grand Canonical Monte Carlo methods may be useful  (Sec~\ref{sec:water_sampling})~\cite{michel2010prediction}. Other tools such as WaterMap~\cite{abel2008role,young2007motifs} or open source equivalents (SSTMap~\cite{haider2018solvation}, GIST~\cite{ramsey2016solvation}, and others) can be used to define the hydration states for systems with no experimental evidence of water sites~\cite{wang2011ligand}. Well-known protein systems with water mediated ligand interactions include HSP90 (which formed part of the D3R grand challenge 2015)~\cite{mey2016blinded}, A2A~\cite{brucemacdonald2018ligand}, MUP~\cite{ross2015water}, ~\cite{deflorian2020accurate}, as well as others~\cite{michel2009energetics}.

Determining the correct hydration pattern for the two endpoints may not be sufficient for an accurate and reproducible calculation of relative binding free energies. If the locations of the bound water molecules differ at the two endpoints (e.g., if a perturbation to the ligand displaces a bound water molecule), then the hydration network must have enough time to change smoothly at each step to the correct equilibrium distribution corresponding to that alchemical intermediate.  Otherwise, it is likely that the estimated relative binding free energies will be in error, such that the results will be dependent on the direction in which the perturbation is performed~\cite{ross2020enhancing}.

In absolute binding free energy calculations, a ligand is fully decoupled from a binding site cavity, which may otherwise have a different hydration pattern in the \textit{apo} state. In this case, the sampling of water molecules must be sufficient to allow the binding pocket to be hydrated as the ligand is decoupled, or dehydrate as the ligand is introduced in the reverse process. When the binding site is exposed and water can move freely between the site and bulk solvent, then sampling is often not an issue. However, in the case of more buried sites or sites which form complex water networks, enhanced water sampling may be required to ensure an accurate binding affinity.

\paragraph{Membrane proteins require additional care}
Integral membrane proteins require embedding in a suitable model membrane to remain stable during simulations. Lipids are usually not included in protein force fields and, although they may be covered by generic small molecule force fields, have often been parameterized separately (e.g., AMBER's Lipid21~\cite{AMBERLipid21} and the CHARMM lipid force field~\cite{CHARMMLipidFF}). Membrane systems can be simulated in multiple thermodynamic ensembles, but the semi-isotropic scheme $NP_{z}P_{xy}T$ is frequently employed, allowing the pressure in the bilayer plane ($xy$) to scale independently to the pressure perpendicular to the bilayer~\cite{LipidBestPracticesCOMS}. These systems require more extensive equilibration than soluble proteins, as lipids move significantly more slowly than water. Some workflows, such as CHARMM-GUI or Desmond membrane preparation, recommend a multi-protocol to relax membrane systems to a reasonable starting point for further simulation~\cite{https://doi.org/10.1002/jcc.23702,DesmondManual}.  Alchemical transformations that involve net charge changes can provide difficulties for membrane systems; phospholipids carry functional groups with nonzero formal charge and cannot be treated as a solvent with a uniform dielectric in the same way as water. Several approaches are reviewed in~\cite{wu2022correction-51c}. The authors recommend the use of a co-alchemical ion, specifically making an ion of equivalent charge appear alongside the disappearing charged molecule to maintain system neutrality.

In the case of a lipid-facing binding site, there are three additional challenges. First, simulation time should be extended to ensure sufficient sampling of lipid configurational space. Lipids may either bind alongside small molecules, acting as cofactors, or be displaced from the binding site by a ligand. If lipids participate in binding through protein-lipid or ligand-lipid interactions, it is necessary to sample these intermolecular interactions through additional $\lambda$ windows, longer simulation times, or different alchemical protocols. Displacing a lipid will have its own free energy cost, and the process may take a long time to converge if it requires diffusion and rearrangement of nearby lipids. Second, considering the free energy cost of a ligand partitioning into the membrane may provide additional insights~\cite{doi:10.1021/acs.jcim.1c01147}. Third, the free energy of binding for a small molecule may be strongly coupled to the lipid species. In some cases, the identity of the lipid may be known. But in other cases, it may be difficult to discern the identity of the lipid from X-ray or cryo-EM structures due to the flexible nature of lipid head groups and benchmarking is recommended to match experimental data.

\paragraph{Protonation states depend on the pH of the experimental assay}
Care should be taken when preparing ligands and proteins to match the pH of the experimental assay, if known.  The calculations aim to capture the alchemical change from one ligand to another as accurately as possible. Therefore, it is necessary to consider details of the experimental setup, such as pH. Biological assays are usually run at neutral pH but this is not always the case. For example, some enzymes exhibit pH-dependent activity and assays may thus be done in conditions other than neutral pH. Therefore, computational protein and ligand preparation protocols should reflect experimental pH. The pH of the assay can differ from neutral pH and will determine the protonation states of the proteins and ligands. Although the pK$_a$ of reference amino acid side chain residues is known, they can vary in the protein environment. A number of different computational tools have emerged for predicting sidechain pKa in proteins, including the H++ server~\cite{anandakrishnan2012automating}, ProPKa~\cite{olsson2011propka3,sondergaard2011improved}, APBS~\cite{jurrus2018improvement}, Chemicalize (\url{https://chemicalize.com/welcome}), and Maestro~\cite{maestro2020maestro}. Strongly acidic (Glu, Asp) or basic (Arg, Lys) sidechains can reliably be predicted to be ionized, but care is still needed as the local environment can modify expected ionization states, such as  the catalytic Asp dyad in proteases. Histidine is notoriously more difficult to predict as its pK$_a$ suggests it ionizes closer to the experimental pH range. For ligands, the pK$_a$ also often needs to be determined, if it is not known experimentally. Accurate pKa prediction for small molecules remains a challenging problem, even with dedicated tools~\cite{isik2018pka}. While often it can be assumed that the protonation state of a ligand and protein will remain the same as the ligand binds, some care needs to be taken with systems where the protonation state may change upon binding~\cite{onufriev2013protonation}. BACE~\cite{kim2015conformational}, for example, famously undergoes a protonation state change on ligand binding. The formal charge and/or tautomeric state of the small molecules can change within a series of analogs, necessitating care in treatment. Additionally, medicinal chemistry efforts might deliberately modify the pKa of a series to modify drug properties, requiring explicit efforts to incorporate these changes into alchemical calculations.

\paragraph{Congeneric series often need alignment}
Input coordinates for a congeneric series may be generated by docking calculations, or by ligand alignment using MCSS algorithms. The latter tends to produce alignments that are more conserved and more consistent free energy changes across a dataset, but will struggle to yield reasonable results for relative binding free energy calculations that involve a significant binding mode rearrangement. This may also lead to steric clashes with the receptor coordinates of the reference ligand if structural rearrangements are needed to accommodate different members of the congeneric series. Small steric clashes may be resolved during subsequent simulation equilibration prior to data collection, but there is a risk that the complex relaxes to a metastable state different from the one experimentally occupied. 
...
In regular biomolecular simulations one studies physical systems or, in other words, all entities in the system, e.g., protein, ligand and waters, represent real molecules. For the following, it is helpful to classify such a simulation system representing a physically realizable molecule as having a {\em chemical} topology. In relative free energy calculations, however, ligand A is transformed into ligand B (see Sec.~\ref{sec:theory-partitioning}). The required alchemical transformations can be realized in several ways, each with distinct advantages and disadvantages. In the first relative free energy calculations, these transformations were achieved with the help of chimeric representations, capable of representing both ligand A or ligand B. The important point here is that there is a single set of coordinates for this chimeric ligand, but its ``topology'' is flexible and can, for the same set of coordinates, represent either end state. We will consider this class of approaches as using an {\em alchemical} topology with a single set of coordinates. The construction of these chimeric intermediates quickly gets complicated and is difficult to automate; therefore, approaches have been developed in which both end states (e.g., ligand A and B) are present simultaneously  but only one is interacting with the rest of the system. Thus, we have dual coordinates, and chemical topologies for each of the entities. In the following, we outline these approaches in more detail, together with additional considerations how the various approaches differ in practical use. While we will point out the respective advantages and disadvantages, we stress that the user's options in practice often are limited by the choice of simulation software.
...
Dual coordinate approaches with chemical topologies encompass a third group of approaches, where essentially two absolute free energy calculations are carried out in opposite directions at the same time.
The so-called ``separated topologies'' (SepTop) method~\cite{rocklin2013separated,Baumann_2023}, the "core-hopping" binding free energy (CBFE) method~\cite{Tsai_JChemInfModel_2026_v66_p1626} and the ``alchemical transfer method'' (ATM)~\cite{azimi2022relative} belong into this category. In SepTop, the two copies are held on top of each other via restraints with respect to the protein/receptor they bind to (as opposed to Restraintmaker, where the molecules are restrained to each other). %In ATM, a restraint is used to keep the bound ligand 1 and the free ligand 2 always some minimum distance apart; during exchange moves, suitable rigid-body rotations/translations are applied to move the free ligand into a position/orientation as similar as possible to the one of the bound ligand.
The CBFE approach is conceptually similar to the SepTop method, although differences lie in the form of the alchemical transformation pathway~\cite{tsai2023amber}, underlying enhanced sampling methods~\cite{lee2023aces,tsai_jcheminfmodel_2024_v64_p7046,zhang_jchemtheorycomput_2024_v20_p3935} and other details of their implementation. 
...
\item Single coordinate (SC) --- Alchemical topology
  ...
  \end{itemize}
For completeness, we list some additional terminology that has been used.

Some studies refer to a ``hybrid'' topology approach to free energy calculations~\cite{gapsys2015pmx, gapsys2016accurate, gapsys2020large}, though in our view these calculations should be classified as single coordinate single topology.  %\sbnote{Check whether this is correct!}. %this term may not yet have achieved widespread use. In this case, "hybrid" seems to indicate that the set up of these free energy calculations involves a hybrid of the two molecules, and much of what is done in these studies uses a single topology approach~\cite{gapsys2020large}.
...
Software packages vary in their use of single or dual topology approaches; for example, AMBER~\cite{Case_JChemInfModel_2025_v65_p7835} formally uses a DC approach but can constrain the positions of selected "common core" atoms to be the same (effectively mimicking the behavior of a SCST for these atoms), while BioSimSpace uses a SCST approach for all engines it supports. Please make sure to check what approach is used with your software package of choice, or whether it supports your choice of approach (GROMACS and GROMOS, for example, support both). 
...
\subsubsubsection{Atom mapping} \label{subsec: Atom Mapping}
Following the choice of system representation and sampling strategy, it might be required to generate an atom mapping. An atom mapping is an important step in order to save simulation cost as it gives insights to users as to which atoms can be treated as the same/similar and which need to be represented independently. The more independent atoms are present, the more simulation cost is to be expected. This is the case especially for system representations using a single coordinate set (Type 1. \& 2), however a dual coordinate set approach could use an atom mapping as well, depending on the sampling method, this will be explained in the next steps.
...
\begin{figure}
    ...
    \caption{\textbf{Illustration of maximum common substructure (MCS) matches between example molecules.} The MCS is shown in green for when (\textbf{A}) a loose MCS match is used where only the methoxy atoms are treated as independent atoms; (\textbf{B}) a restrictive MCS match is used where all atoms are treated as independent atoms, meaning there is no mapped MCS match between the two compounds.}
    \label{fig:fig_mcss}
\end{figure} 

In general, there are many ways to calculate an MCSS. One of the most prominent approaches is to estimate the MCSS with a 2D subgraph isomorphism solver(SIS) using the 2D topological structure information of molecules~\cite{raymond2002maximum}.
A large number of software tools can compute SIS MCSS matches using different cheminformatics packages. Some rely on RDKit~\cite{rdkit2019Dec}, including pmx~\cite{gapsys2015pmx}, LOMAP~\cite{liu2013lead}, SMArt~\cite{petrov2021perturbation}, and, in part,  BioSimSpace~\cite{hedges2019biosimspace}, while others such as fkcombu~\cite{kawabata20143d} are standalone tools. Schr\"{o}dinger's FEP+ planning tool was originally based on a version of LOMAP, and but currently also uses MCSS matching as well as 3D considerations to plan the network of single topology calculations between molecules~\cite{wang2015accurate}. 

SIS-based methods were shown to struggle with 3D properties of the molecules that need to be mapped, for example, ligand stereochemistry. Therefore, some packages developed a post-mapping step, where they filtered for atom distances, and, therefore, added 3D information into the mapping.
SIS MCSS searches also can be relatively time consuming as the algorithmic complexity is classified as NP-complete, so if the goal is to assess a library of ligands to identify promising pairs for relative calculations, it can be helpful to use faster approaches such as shape similarity to perform an initial similarity assessment and then use MCSS only to identify final mappings for relative calculations~\cite{raymond2002maximum,klabunde2012mars,jones2009elucidating, Duesbury2018Comparison}. The SIS MCSS approach, though relatively standard, takes into account only topological similarity. It is possible that changes in binding mode could actually require a different choice of mapping, so in some cases mappings may need to be planned differently depending on 3D positioning of atoms in space. 

An alternative way to estimate the MCSS is to calculate a bipartite graph matching based on the 3D positions of two molecules. These approaches tend to be very fast (can have algorithmic complexity of $O(n)=N^3$) and geometrically accurate in their mapping (e.g. stereochemistry or binding mode differing).
...
A disadvantage of these approaches is the strong dependency on the coordinate sets being well aligned. Another drawback of the approach is not taking the topology changes of the molecules into account, which can be done if necessary in a next step with a filtering approach, such as checking for changes in ring hybridization or ring breaking (this problem is discussed below in Sec.~\ref{subsec: Ring breaking and forming}).
...
Single coordinate set relative calculations and calculations based on substructure searches are generally only if in fact the ligands share a common substructure, e.g. are part of a congeneric series, see Fig.~\ref{fig:fig_mcss}.
If no common substructure is shared, then dual coordinate sets methods like linked-dual or separated topology free energy calculations are needed. In such approaches, one can co-localize a pair of compounds in a binding site, exclude their interactions with one another, and compute the relative binding free energy by turning one molecule on from being dummy atoms while turning the other off.
It is crucial for these approaches to find a good set of restraints that keep the ligands colocalized over the time of the simulation. This problem is similar to the required restraints in ABFE Calculations (see \ref{sec:standardstate-restraints} ). Few descriptions of possible approaches in RBFE calculations can be found for such approaches like the one suggested by Baumann et al.~\cite{baumann2023broadening} based on Boresch-style restraints~\cite{boresch2003absolute}, with an elaborate automatic atom selection approach, for the separated topology approach, or Restraintmaker~\cite{ries2022restraintmaker} for the linked-DC approaches, which finds a small set of atoms to be linked via distance restraints, with a small distance to each other. 
However, the implementation and testing of a fully automatized dual-topology pipeline for RBFE dual topology approaches is still to our knowledge a remaining research problem and was only tested with small datasets.~\cite{jespers2019qligfep, rieder2022lev}

\subsubsubsection{Dummy atoms}\label{subsec: Dummy Atoms}

In practically all relative free energy simulations, regardless of the approach, SCST, SCDT, or DCDT, dummy atoms (non-interacting placeholder atoms) are required (see Fig.~\ref{fig:fig_topology}). We stress this point, as sometimes dummy atoms are viewed as something specific to SCST. In SC approaches, dummy atoms may even be required if the number of atoms between initial and final state of the system is identical, for example, when modeling multiple tautomeric forms of a molecule.

%The need for and role of dummy atoms is clearly visible in Fig.~\ref{fig:fig_topology}: One or more dummy atoms are attached to the physical molecule. 
A crucial requirement is that these atoms, typically connected through bonded energy terms (bond stretching, angle bending, dihedral angles) do not affect equilibrium and dynamical properties of the physical system. However, since a physical molecule + dummy atoms attached to it can or must be viewed as a modified physical molecule, the partition function of the molecules with and without dummy atoms will NOT be identical. In other words, a single free energy difference between two molecules A and B will be different from that of A(+D) and B(+D), where the (+D) indicates the presence of one or more dummy atoms needed so that the alchemical transformation can be carried out. When a dual coordinate approach is used, the noninteracting part of the system is tied to the configuration of the interacting part of the system, and will contribute to the partition function. By construction, relative free energy simulations involve two analogous alchemical transformations, e.g., A(+D) -> B(+D) in gas phase and aqueous solution (RSFEs), or in the bound and unbound state (RBFEs). If dummy atoms  interact with the physical system in such a way that $\mathsf{Z = Z(R) \times Z(R\text{-}D) \times Z(D)}$, where Z is the total partition function, Z(R) the partition function of the physical system, Z(D) the partition function contribution resulting from interactions between only the dummy atoms, and Z(R-D) the contribution resulting from the bonded connections between the physical atoms and dummy atoms, then their contributions (Z(D) and Z(R-D)) factor out from the total partition function. As dummy atoms do not interact with the environment, their contribution, therefore, is identical in the two alchemical transformations required in the two legs of the relative transformation and will cancel exactly from the double free energy difference of interest.

While some early work on the correct treatment of dummy atoms, i.e., how to treat them so that their contribution to the partition function factors, has been available for years~\cite{Boresch_2002,Shobana_2000,Wang_2012}, most workers seem to turn off nonbonded interactions for dummy atoms, but leave all bonded interactions intact. Since current force fields use multiple angle and torsion terms for each atom, there are often a large number of bonded energy terms connecting dummy atoms to the physical molecule. However, as already described in the early studies and repeated in a more recent study focusing on the correct treatment of dummy atoms~\cite{Fleck_2021}, the desired factorization can only be guaranteed if any dummy atom connected to the physical molecule is held by exactly three (non-redundant) degrees of freedom. Naive application of the three-degree-of-freedom per dummy atom rule (for dummy atoms having at least one joint bonded term with the physical molecule), however, can result in simulations where the dummy part adopts unphysical configurations/conformations; Fleck et al.~referred to this as ``flapping''. E.g., when mutating methane to ammonia in an SCST approach, one of the methane hydrogen atoms becomes a dummy atom. According to the three-degrees-of-freedom rule, it should be connected to the nitrogen only by one distance and two bond angle terms. However, these three internal coordinates permit the dummy atom to adopt several equivalent positions in Cartesian coordinates. In simulations one will find the dummy atom not only at the position of the former hydrogen atom, but also other positions. Indeed, it could even have a position overlapping with one of three ammonia hydrogens.   Thus, depending on the details of the physical molecule-dummy atom(s) junction, the three degrees of freedom need to be selected with care, or a carefully chosen additional redundant degree of freedom may be needed. In the latter case, the factorization of the partition function is not exact, but the overall error can still be made arbitrarily small, at least for the systems tested in Ref.~\cite{Fleck_2021}.

One situation involving dummy atoms that needs to be avoided is the creation of a cyclic structure involving dummy atoms.  Specifically, this would occur whenever the dummy atom or atoms are attached to two different parts of the physical molecule. In this case, the factorization of the partition function is never possible~\cite{Shobana_2000}. Ring opening or closing is thus extremely challenging to carry out using SCST approaches~\cite{liu2015ring}; these difficulties have in part motivated the development and adoption of dual coordinate approaches. (For more on ring opening/closing, see also Sec~\ref{subsec: Ring breaking and forming} below on ``Ring breaking and forming''). Nevertheless, DC approaches are susceptible to exactly this type of problem: to keep the noninteracting molecule on top of the interacting one, typically multiple bonded restraints (e.g. harmonic bonds) are used~\cite{Axelsen_1998,Ries_2022}. These effectively, are equivalent to the formation of cyclic structures with potential erroneous contributions to the double free energy difference of interest. The type of restraints used in SepTop~\cite{rocklin2013separated,Baumann_2023} and ATM~\cite{azimi2022relative} avoid this problem.
...
This incorrect treatment of dummy atoms (leaving all bonded terms intact throughout the alchemical transformation) introduces a system-dependent bias which is difficult to estimate, though it is likely small in many cases~\cite{Fleck_2021}. Nevertheless, since the degree of bias is both generally unknown but also avoidable, some guidelines should be followed:
\begin{itemize}
  \item Obviously, a system with and without dummy atoms should have the same optimized geometry; practitioners should test this as follows. Optimize the geometry of the physical system (no dummy atoms); the final energy is the reference value. Next optimize the geometry of the same molecule in the presence of dummy atoms; the final energy may well be different from the reference energy due to bonded interactions between the dummy atoms. Now delete the dummy atoms and recompute the energy \underline{without} further minimization of the physical atoms. The energy after the deletion of the dummy atoms must be identical to the reference energy. 
  \item The above is only a \underline{necessary} criterion, and the dummy atoms may still influence the dynamical behavior of the physical molecule. Furthermore, overzealous removal of redundant energy terms may result in flapping of the dummy atoms. If one suspects the latter, then a short simulation in the gas phase will reveal flapping by visual inspection. 
  \item   Note that in larger groups of dummy atoms, redundant bonded terms involving only dummy atoms will be essential to avoid flapping. In such cases, the energy of the optimized geometry in the presence of dummy atoms may be quite different from that of the physical system without dummy atoms. 

  Unfortunately, while straightforward in principle, both checks described above (energy calculation with and without dummy atoms after geometry optimization, visual inspection for flapping) are hard to automate, which is essential in large RFEs campaigns.
  \item The correct treatment of dummy atoms can be validated by closing a suitable thermodynamic cycle.  E.g., Fleck et al.\ calculated ASFEs for each of their endstates, as well as the RSFE between them~\cite{Fleck_2021}. In all cases when dummy atoms were treated according to best practices, the cycle closure error was well within the statistical uncertainty, which was well below 0.1 kcal/mol. This contrasts with a related, earlier study by Loeffler et al. who also reported ASFEs and RSFEs. If one attempts to close the cycle based on the numbers reported there, cycle closure error was nonnegligible in some cases~\cite{loeffler2018reproducibility}. 

  \end{itemize}
%
Further work automating the correct setup and treatment of dummy atoms is necessary (see also Sec.~\ref{sec:unsettled}); the package \texttt{ghostly} (Sec.~\ref{sec:software}) is a step towards this~\cite{hedges_ghostly_2025.2.0}.


\subsubsubsection{Ring breaking and forming} \label{subsec: Ring breaking and forming}
...
Some approaches have attempted to address this using a single coordinate sets and with soft bond approaches ~\cite{clark2019relative,wang2017accurate} or dual coordinate set approach~\cite{jespers2019qligfep, ries2022relative, bhati2017, bieniek2021} but a general solution is not yet in mainstream use. Challenges with ring breaking and forming can involve some of the same issues with dummy atoms as discussed above in the ``Dummy Atoms'' discussion, with some additional complexities. Still, FEP+ implements one solution\cite{wang2017accurate}.

\subsubsubsection{Constraints and relative free energy calculations}
One issue which requires particular care is the use of constraints.
It is common in biomolecular simulations for bonds involving hydrogen are constrained to a fixed length using algorithms such as SHAKE or LINCS, rather than being allowed to vibrate.  This approach enables the use of longer timesteps~\cite{krautler2001fast}, usually up to 2 times. 
However, in single topology relative free energy calculations, the atoms involved might be mutated to other atom types---for example, in a mutation of methane to methanol, one hydrogen might become an oxygen atom. The bonds with such atoms might not have any constraints, or if all bonds are constrained, have constraints of different lengths. 
Some molecular dynamics engines are not set up to recognize this change, or at least not to correctly include contributions to the free energy from changing constraints/constraint length, so results for a transformation including constraints can be erroneous.
...
As discussed in Sec.~\ref{sec:theory-solvation} and \ref{sec:theory-partitioning}, handling the standard state in solvation and solvent partitioning free energy calculations is typically straightforward. Eq.~(\ref{eq:DG0-solvation-def}) identifies the ideal and excess components of the standard solvation free energy in terms of the standard gas pressure $P^\circ$, standard solution concentration $C^\circ$, and ensemble averages typically evaluated by alchemical free energy simulations. These relations provide simple recipes to connect computational estimates to experimental values obtained under different conditions. Moreover, typical reported experimental data (Henry's constants and partitioning coefficients) directly probe the excess term calculated by molecular simulations, which is independent of the standard state.

As also discussed above in Sec.~\ref{sec:non-covalent-molecular-binding} and \ref{sec:theory-abfe}, the situation for binding is more complex and requires special care. The first important aspect is that binding free energy calculations must be performed with binding site restraints that effectively define what the binding site is, forcing the ligand remain within the defined binding site even when it is decoupled from the receptor. Doing otherwise would fail to specify the bound macrostate of the complex, and results in undefined binding free energy estimates. However, in practice, calculations lacking binding site restraints often, though not reliably, return reliable binding free energy estimates due to the kinetic trapping of the ligand in the binding site region during the relatively short molecular dynamics sampling typical of alchemical binding free energy simulations. Nevertheless, longer calculations, especially of weakly bound ligands, could display persistent drift of the estimate as new regions of conformational space are sampled. A more serious consequence of neglecting binding site restraints is the possibility of the ligand leaving the binding site when it is not coupled to the receptor. Alchemical binding free energy simulations where the ligand leaves the binding site are not consistent with the theory of bimolecular binding, and can therefore return grossly inaccurate binding free energy estimates.

Mathematically, the binding site is identified by use of an indicator function in collective coordinates $I(\zeta)$, as discussed in Section~\ref{sec:non-covalent-molecular-binding}. Because $I(\zeta)$ is a discontinuous function of the position and orientation of the ligand relative to the receptor, molecular dynamics implementations often approximate the indicator function by means of flat-bottom restraints~\cite{chen2007can, wu2021alchemical} (see below). Harmonic restraints are not as suitable in this respect because they favor a specific microstate at the bottom of the harmonic potential rather than specifying a region of conformational space.
...
For example, the overall ligand RMSD has been used~\cite{woo2005calculation} for this purpose.
The Boresch variables~\cite{boresch2003absolute,baumann2023broadening} are a popular choice for building auxiliary restraints because they are based on interatomic distances, bond angles, and dihedral angles degrees of freedom already implemented in molecular mechanics codes and do not interfere with intramolecular conformational distributions. Moreover, the free energy of releasing Boresch restraints within the simulation box volume $V$ is easily computed analytically~\cite{boresch2003absolute, boresch2024analytical}.

Auxiliary restraints are designed to reduce the amount of conformational sampling at intermediate $\vec{\lambda}$ values. However, more computational effort is required to compute the free energy when the restraints are turned on and off. Additionally, such restraints can limit the exploration of alternative binding modes. This restriction may be undesirable when using Hamiltonian $\lambda$ exchange or expanded ensemble techniques, where allowing the ligand to exchange binding modes when it is non-interacting could provide sampling benefits~\cite{wang2013identifying}.
More specifically, using binding site flat-bottom restraints alone allow a ligand to explore multiple binding sites, and harmonic auxiliary restraints in ligand-binding site distances allow the exploration of multiple binding modes within a site. In contrast, full Boresch restraints on distance, angles, and torsions only allow a single binding mode within a single site.
...
According to the non-covalent statistical mechanics theory of bimolecular binding (Sec.~\ref{sec:theory}), only the conformations in which the receptor and ligand form a bound complex are sampled. Determining an appropriate definition of the bound macrostate can be challenging for weakly bound ligands. For tightly bound ligands, virtually all reasonable definitions of the bound state will lead to equivalent free energies, because the partition function will be dominated by a relatively limited ensemble of low-energy poses contained with all definitions of the binding site. However, this simplification breaks down for weak binders.  In fact, there might not be a perfect correspondence between the structural definition of the complex on which current alchemical models are based~\cite{gilson1997statisticalthermodynamic} (Sec.~\ref{sec:theory}) and experimental reporters~\cite{mihailescu2004theory}. For example, isothermal titration calorimetry (ITC) or surface plasmon resonance (SPR) measurements effectively define a binding state that encompasses all ligand conformations complexed with the protein, regardless of where they bind the protein. In contrast, fluorescence polarization competition assays measure binding at a single location, in which the ligand of interest displaces a competing binder. Therefore, care must be taken to ensure that a reasonable definition of the binding site is used~\cite{wang2013identifying}.

Apart from the difficulty of selecting an appropriate definition of the binding site region that is consistent with the experimental technique, the alchemical theory and computational procedures are the same whether strong, weak, or non-binders are considered. The alchemical binding free energy calculation will report the binding free energy for the selected binding pose specified, for example, by the region allowed by flat-bottom binding site restraints. Changing the definition of the binding pose will yield a binding free energy for the new pose, which can often differ from the binding free energy of any initial pose. 

Simulations of weak binders are likely to result in the ligand exploring the boundaries of the allowed binding site region. In such occurrences, which can also occur even for a tight binder when the allowed binding site region is too restrictive, or the ligand is attempting to access a lower free energy pose than the initial pose, the choice of the binding site restraints will have a significant effect on the binding free energy estimate. However, in practice, alchemical binding free energy models are typically employed to identify binders and optimize the affinity of strong binders. Hence, it is often not critical to quantitatively determine the binding free energies of weak and non-binders, nor to assess their dependence on the binding site definition.

\subsubsection{Changes in net charge can be challenging/problematic}
\label{subsec:net charge}
Changes in net charge of the system along the alchemical transformation can pose major challenges for unbiased, reliable free energy estimation.
Specifically, finite-size effects can introduce significant charge-dependent artifacts into computed binding free energies, in part because typical schemes for long-range electrostatics, including particle mesh Ewald (PME) and reaction field, do not handle free energy contributions from such changes as they would be handled in a hypothetical macroscopic bulk solution. The resulting free energy for these methods  depends either on the size of the cutoff (when using reaction field methods) or on the size of the simulation box (when using lattice sum methods, due to periodic boundary conditions)~\cite{lin2014overview, ohlknecht2020correcting, rocklin2013calculating}.

Lattice summation methods (e.g. PME) effectively neutralize the simulation box by applying an implicit homogeneously distributed background counter charge~\cite{figueirido1995finite,hummer1996free}. The background charge can be interpreted as a converged distribution of counterions and may be a reasonable choice for computing hydration free energies of monovalent ions. Finite size effect corrections should be applied in these cases~\cite{hummer1996free}. In inhomogeneous systems with spatially-changing dielectrics, e.g. a solvated protein or lipid bilayer in a water box, assuming uniform background countercharge distribution is unphysical, as the higher countercharge density should be located in the regions of higher dielectric~\cite{hub2014pme}. This, in turn, causes significant artifacts in sampled population densities of charged molecules, and thus errors in calculated free energy differences.

There are two main classes of approaches to address artifacts introduced by changes in net charge during alchemical free energy calculations: (1) avoiding changing the net charge altogether and (2) post-processing approaches to correct for artifacts using continuum electrostatics.
...
Although often water sampling is less of an issue in ABFE calculations, as water is more free to enter the binding site when the ligand is absent, handling changes to water networks in buried pockets during ligand coupling/decoupling can be addressed in a similar fashion to RBFEs with grand canonical methods.
...
There is currently no single best practice for handling slowly sampling waters, however it is recommended that the careful use of some combination of the techniques described above should allow the water network to adapt during the perturbation of the ligand, ensuring more accurate and converged affinity estimations that are independent of the starting configuration and direction in which the calculation is performed.
...
Both absolute and relative calculations must choose an alchemical pathway connecting the initial and final states. In principle, because of the path independence of the free energy, any arbitrary pathway will give the correct free energy change.  However, he choice of pathway taken will greatly affect the efficiency of the calculations. Some choices are particularly crucial.  For example, alchemical pathways must avoid ``bare'' infinitely attractive potentials, such as the Coulombic interaction between opposing charges, unless they have an even more rapidly increasing infinitely repulsive term present.  This is because the bare attractive term will result in particles rapidly treating each other as, essentially, black holes, which leads to immediate simulation crashes if contact occurs.  And yet at the same time, transformations involving insertions or deletions of infinitely repulsive terms such as the Lennard-Jones interaction should employ methods (described in more detail in this section) such as a soft-core potential path, as naive choices to bring the potential down from infinity result in highly biased results~\cite{beutler1994avoiding, beutler1994molecular,gapsys2012new, Lee_JChemTheoryComput_2020_v16_p5512}. 
...
For many energy terms this is a very good approach, \textit{as long as a repulsive core remains on}. For example, it can be shown that if van der Waals repulsions are left on, then the linear approach is very nearly the optimal path possible for changing, removing, or inserting the electrostatic energy terms, with the alchemical path being within about 10--20\% of the minimum possible uncertainty~\cite{naden2015linear} for a fixed amount of simulation time, as well as being nearly optimally efficient for van der Waals attractive terms with repulsion terms turned on~\cite{naden2014linear}. Although we are not aware of any quantitative tests for a linear transformation for dipolar or higher multipole terms, theoretically it should behave equally well for those types of interactions as well.

However, this linear approach ends up being terrible for removing or adding repulsive potentials that go to infinity very quickly at or near the origin. One way to see this problem is to examine how low $\vec{\lambda}$ values must go to reduce the energy at $0.5\sigma$ (the atomic size parameter) down to 1 $k_BT$, where thermal fluctuations make it possible for other atomic sites to easily penetrate routinely that deep. Assume we are trying to go from a particle being present, and desire to make it disappear alchemically. If the repulsive terms are of the form $\epsilon(\frac{\sigma}{r})^{12}$, and if $\epsilon$ is 1 $k_BT$ at the temperature of interest, and we start with the particle present, we may solve for $(1-\vec{\lambda})(1 k_B T)\left(\frac{\sigma}{0.5\sigma}\right)^{12} = 1 k_B T$. This yields $\vec{\lambda} = 1-2^{-12} \sim 0.999976$. At this point,  we have gone virtually all the way to the end of the transformation, but there is still an impenetrable sphere in the middle of our simulation that we are trying to disappear! This is not very much like the desired final state of no interactions between the particle and its environment. One can play around with a few ways of improving the convergence, like simulating many more intermediate states near $\vec{\lambda}=1$. However, various analyses have shown that this is not a very good strategy~\cite{pham2011identifying, beutler1994avoiding, zacharias1994separationshifted, blondel2004ensemble, gapsys2012new}.

What we need instead is a function that smoothly removes this infinity. A large number of schemes have been tried~\cite{beutler1994avoiding, zacharias1994separationshifted, blondel2004ensemble, pham2011identifying, pham2012optimal, naden2014linear, donnini2005incorporating,isthereagapsysone?}, but the most common strategy is to use a ``soft-core'' potential, of the form:
...
where $r_{ij}$ is the distance between two particles $i$ and $j$, $\epsilon_{ij}$ and $\sigma_{ij}$ are the Lennard-Jones parameters corresponding to the interaction between particles $i$ and $j$, and $\alpha$ is a constant. In particular, $\alpha=0.5$ is statistically optimal for the specific functional form shown above. This functional form has exactly the properties we are looking for: it recovers the Lennard-Jones potential when $\vec{\lambda}=1$, and the at other endpoint ($\vec{\lambda}=0$), it smoothly goes zero for all $r_{ij}$ everywhere when $\vec{\lambda}$ goes to zero, as the $\alpha(1-\vec{\lambda})$ term lowers the infinite energy in the core. 

There are several different variants of the same functional form~\cite{zacharias1994separationshifted, beutler1994avoiding,pham2011identifying,Lee_JChemTheoryComput_2020_v16_p5512,tsai2023amber}, but the one given in eq.~\ref{eq:softcore} is easy to understand and implement and fairly numerically stable. This functional form is shown in \textbf{C} and \textbf{D} of Fig.~\ref{fig:fig_what_is_lambda}.  One can derive a near minimum variance of pairwise pathways, and can show that all of these forms forms are relatively similar in statistical precision and efficiency and near the global optimum ~\ref{eq:softcore}~\cite{pham2012optimal} when disappearing Lennard-Jones terms in the absence of charge terms. We therefore mildly recommend using the soft-core potential given in eq.~\ref{eq:softcore} for simplicity. However, if your chosen software supports using any similar flavor equation to eq.~\ref{eq:softcore}, it will almost certainly be acceptable and the efficiency in terms of amount of simulation required will be relatively similar.  Again, however, if you are inserting or removing entire atomic sites, we heavily recommend against using the linear approach; it will be very difficult to get correct or converged results. 

So far in this section, we have discussed optimal ways of disappearing or appearing Lennard-Jones interaction sites and turning on and off electrostatics terms. What about performing both transformations at the same time? We cannot turn off the electrostatics linearly at the same time we turn off the Lennard-Jones terms, as it would leave infinitely large attractive and repulsive electrostatic terms ``bare'' at small $\vec{\lambda}$, resulting in the simulation crashing. It \textit{is} possible to apply the same soft-core approach to the Coulomb interaction as to the van der Waals interaction, and this is indeed done in a number of implementations. In this case, it is important that the Coulomb interaction is softened as rapidly or more rapidly than the Lennard-Jones interaction to avoid charge penetration issues into the repulsive core, which  can be tricky to ensure for multiple types of perturbations simultaneously~\cite{steinbrecher2011softcore}. Tsai et al.~\cite{tsai2023amber} show how separate soft-core potentials can be used for electrostatic and Lennard-Jones terms, and appropriately balanced such that they avoid end-point instability, particle collapse and large gradient jump problems~\cite{tsai2023amber}.
...
through either annihilation~\cite{hermans1997inclusion, mann2000modeling, boresch2003absolute, wang2006absolute, mobley2006use} or decoupling~\cite{fujitani2005direct, mobley2006use}, must be considered. Reasonable efficiency can be often obtained with either choice even if some are somewhat better or worse than others, and there is no consensus on which is better in most given situations. Our recommendation is to leave the intramolecular interactions on during the transformation for simplicity if there are no other known issues with this approach.  It can be considered the ideal gas state, as all interactions within the molecule are present, but no interactions with other molecules are.  However, there are a few cases where at least some of the intramolecular interactions should be turned off.  For example, if intramolecular hydrogen bonds can be easily formed, they can lock the uncoupled or near uncoupled states into a long-lived metastable hydrogen bonded state.  This can be avoided by turning off the electrostatics in the uncoupled state, or by using sampling technique like expanded ensemble or replica exchange that allows escape from this sort of metastable trap.  The key feature of the simulation to watch out for is whether the total potential energy, and therefore the intermediate ensembles sampled, changes smoothly from beginning to end.  Trapped states occurring only at the uncoupled end state are often also easily identified by visualization of simulation trajectories. Problems of discontinuous changes of the potential energy can be diagnosed by noticing lack of configuration space overlap between different simulations (see Sec.~\ref{sec:are-they-good}).


Relative calculations introduce additional choices for choosing intermediate states, such as the order in which to modify nonbonded interactions.
...
For BAR/MBAR techniques this means that states should be spaced so that the statistical uncertainties between neighboring states be approximately equal~\cite{pham2012optimal, shenfeld2009minimizing}, where ``approximately'' is with all of the statistical uncertainties are roughly within 30-50\% in magnitude. 
Some schemes to adaptively optimize the spacing of intermediate states based on initial exploratory simulations have been proposed~\cite{hayes2017adaptive,zhang_jchemtheorycomput_2024_v20_p3935}, including methods based on optimizing phase space overlap or uniform replica exchange acceptance ratios~\cite{zhang_jchemtheorycomput_2024_v20_p3935} that are correlated with statistical uncertainties. For molecules changing in dense solvent, then the best path is roughly independent of molecule size and shape, so what works for one molecular transformation is likely to be relatively efficient for another~\cite{monroe2014converging}.
...
%MRS: note to myself - put in something around here about how short-range cutoffs and LJ relate, and also about PME LJ, including discussion from doi:10.1021/acs.jctc.5c00646. 
...
Though all alchemical simulations must sample from multiple $\vec{\lambda}$ states, different approaches can be used to achieve this sampling. Fig.~\ref{fig:fig_sampling_scheme} illustrates the four most common schemes. The simplest approach involves running an independent simulation at each of the predefined $\vec{\lambda}$ values (see Fig.~\ref{fig:fig_sampling_scheme}\textbf{A}). This type of scheme is currently supported by most contemporary MD engines.

However, if these simulations at different $\lambda$s can be run simultaneously with communication between the simulations, a simple extension allows \textit{mixing} between these replicas. In this approach, the simulation at each $\vec{\lambda}$ can undergo periodic exchanges with neighboring $\vec{\lambda}$ values. This form of replica exchange, called \textit{Hamiltonian replica exchange}, is based on ideas developed from Monte Carlo simulations of spin glasses by Swendsen and Wang~\cite{swendsen1986replica}. With the Metropolis-Hastings acceptance criterion for exchanges, the generated ensemble of all replicas still samples from the Boltzmann distribution for each replica. This approach has been used in many different contexts for molecular simulations~\cite{sugita2000multidimensional,sugita1999replicaexchange, woods2003development, jiang2010free}. The basic idea of the replica exchange scheme is shown in Fig.~\ref{fig:fig_sampling_scheme}\textbf{B}. It is supported in various software packages that provide alchemical implementations, such as GROMACS~\cite{aldeghi2015accurate}, GROMOS~\cite{hritz2008hamiltonian,hritz2007optimization},
AMBER~\cite{Lee_JChemInfModel_2020_v60_p5595,Case_JChemInfModel_2025_v65_p7835}, FEP+~\cite{wang2015accurate}, NAMD~\cite{jiang2019computing}, and OpenMM~\cite{rufa2020chemical,azimi2022relative}.

With Hamiltonian replica exchange, one can include additional differences in the Hamiltonians besides just changing $\lambda$, in ways that improve sampling, as long as the end states remain the proper physical states. Such a variant of Hamiltonian replica exchange is FEP/REST2~\cite{rest2, feprest} that involves ``heating'' a small portion of the protein-ligand system to enhance the sampling within the binding pocket. It has been promoted as a great way to enhance sampling and is employed by default in Schr\"{o}dinger's FEP+ package. However, it should be noted that several studies have questioned its reliability and reported that it can degrade the quality of free energy predictions as compared to those based on standard MD simulations without any enhanced sampling~\cite{wan2020fep+, bhati2022}. Therefore, a blind application of this sampling scheme is not recommended and it must be used with caution. 

A variant of this approach of changing intermediate state Hamiltonians to improve sampling that overcomes some of the reported issues with FEP/REST2 is called alchemical enhanced sampling (ACES)~\cite{lee2023aces}. ACES creates localized targeted ``enhanced sampling states'' that are connected to the real state end-points through HREMD.  The method leverages the dual topology framework to produce a concerted ``counter-diffusion'' of alchemical states that leads to minimal rearrangement of the environment along the alchemical path~\cite{York_ACSPhysChemAu_2023_v3_p478}. The computational overhead of the method is negligible relative to more conventional alchemical free energy simulations with HREMD.

A third approach borrows ideas from simulated tempering~\cite{marinari1992simulated}. In this scheme, a single replica rapidly explores all of $\vec{\lambda}$ space by switching between different intermediate $\vec{\lambda}$ values, as seen in Fig.~\ref{fig:fig_sampling_scheme}\textbf{C}.  This is referred to as expanded ensemble, extended ensemble or even just simulated tempering (although the temperature is not changed when applied to $\lambda$ changes in alchemical free energy calculations).~\cite{lyubartsev1992new, li2007simulated, tan2017optimally} and is supported OpenMM Tools~\cite{andrearizzi2019choderalab}, GROMACS and AMBER TI~\cite{Lee_JPhysChemB_2025_v129_p13063}.    One can show that the expanded ensemble calculations will move back and forth between replicas faster than replica exchange.~\cite{Zhang:JCP:2008}  

Although this approach allows multiple states to be simulated in a single simulation, thus removing requirements for multiple parallel simulations, in order to sample all of the states evenly, one must introduce a weight for each state; otherwise, the simulation will remain in the states that are near the free energy minimum. The weights that give exactly equal sampling of all states are just $\exp(-\beta F)$, meaning that one must know the free energies themselves to estimate the free energies!  Since any reasonable estimate of the free energies will give approximately equal sampling of all $\lambda$s, leading to good free energy estimations, the problem is not quite as bad as it first appears. A number of approaches have been proposed to converge the weights more rapidly to the equal sampling weights, including Wang-Landau sampling~\cite{Wang:PRL:2001} and self-adjusted mixture sampling~\cite{tan2017optimally}, and even using metadynamics in $\lambda$ space~\cite{Hsu:JCTC:2023}. A full description of the different methods to adjust the weights is beyond the scope of this review---the interested reader is directed to a more complete review in~\cite{Henin:LJCMS:2022} of the different choices as well as the other $\lambda$-based enhanced sampling methods.  However, at this point, all of these methods require some effort and attention to robustly give equal sampling to all $\lambda$ states and give consistent free energies.
...
In a discovery setting, new ligands can have unknown or at least uncertain binding modes~\cite{kaus2015how, plountprice2000analysis,mobley2009binding,calabro2016elucidation}, which complicates estimating the binding free energy.
...
In some cases, it is tempting to simply use docking techniques to generate initial bound structures for starting molecular dynamics simulations. Docking techniques are good at identifying sterically reasonable potential binding modes, but still perform relatively poorly at identifying which of several allowable binding modes is the most dominant \emph{a priori}~\cite{macdermott-opeskinComputationalCommunityBlind2026}. Such allowable binding modes often represent metastable states. The timescales for binding mode interconversion between metastable binding states are usually slow compared to MD/free energy timescales, meaning that simulations started from different potential binding modes are likely to yield disparate computed binding free energies~\cite{mobley2006use, palma2012computation, mobley2012perspective, gill2018binding}. 


It is worth highlighting a SAMPL blind challenge on HIV integrase as an illustration of the difficulty of performing reliable free energy calculations without clear knowledge of the binding site. 
...
Routes to handle multiple potential binding modes differ depending on whether absolute or relative calculations will be performed. If a method is available to estimate the relative populations of different stable binding modes in advance (e.g. such as the BLUES approach currently in development~\cite{gill2018binding}), such an approach could be applied to assist both types of calculations.

\paragraph{Handling multiple potential binding modes within absolute binding free energy calculations.}
When performing absolute binding free energy calculations, multiple potential binding modes can be handled by two main strategies: Considering each binding mode separately (a separation of states strategy) or sampling all binding modes within a single simulation~\cite{mobley2012perspective}.
This choice also affects the choice of restraints selected, as some types of restraints will allow transitions between binding modes and even binding sites (Sec.~\ref{sec:standardstate-restraints}), and others do not.

Sampling all potential ligand binding modes within a single free energy calculation is usually impractical without some form of enhanced sampling such as Hamiltonian replica exchange~\cite{wang2013identifying} because barriers for binding mode interconversion result in kinetics which are generally too slow compared to simulation timescales~\cite{mobley2006use, palma2012computation,mobley2012perspective, gill2018binding}.
...
The ``separation of states" approach provides a simple, though potentially expensive, alternative.  In this approach, each stable binding mode is considered separately with a binding free energy calculation restricted to that binding mode, and then, as long as the binding modes are non-overlapping, the resulting component binding free energies can be combined into a total complex free energy~\cite{mobley2006use, mobley2012perspective}.
This approach requires a separate binding free energy calculation for each potential binding mode, however, so it can be computationally quite costly.
...
For example, one could compute the free energy of turning off the methyl group in one orientation, and then turning it back on in the other orientation to obtain the free energy difference between the two potential binding modes.
While this approach has precedent and has been used prospectively for molecular design~\cite{desimone2019}, it is relatively difficult to automate at present and requires considerable care.

Overall, means that relative free energy binding calculations will often be susceptible to problems resulting from uncertainty in ligand binding modes unless more robust approaches are available to determine dominant binding modes, or the relative populations of different potential binding modes, in advance.

\subsubsection{Absolute and Relative Binding Free Energies with Multiple Chemical States}
...
In summary, the observed binding free energy of a compound that exists in multiple chemical forms (protomers, tautomers, etc.) can be modeled as the combination of the binding free energies of each form. The combination coefficients correspond to the population of each form in solution. While it is generally not trivial to obtain them from first principles, the populations of the ligand's chemical states can often be derived from experimental quantities, such as pKa values. The combination procedure for relative binding free energies is more involved but follows the same principles. 

A rare chemical form of the ligand in solution can still contribute significantly to the overall binding affinity if its binding free energy is more favorable than that of the others. Conversely, the estimate from a single high-affinity form is likely to overestimate the experimental affinity unless it is weighted appropriately by its solution population. 

The analysis described here is recommended whenever a ligand is expected to exist as a mixture of chemical forms with detectable concentrations in solution. While we focused on the ligand, the principles outlined here apply equally well to the receptor. 
...
------
The free energy difference $\Delta F_{01}$ between an initial state 0 and a final state 1 in a canonical ensemble defined by two Hamiltonians $H_0(\vec{q},\vec{p})$ and $H_1(\vec{q},\vec{p})$ can be calculated as:
\begin{eqnarray}
\Delta F_{01} & = & -\beta^{-1}\ln \expect{e^{-(H_1(\vec{q},\vec{p}) - H_0(\vec{q},\vec{p}))}}_0 =  -\beta^{-1}\ln \expect{e^{-\Delta H(\vec{q},\vec{p})} }_0
\end{eqnarray}\label{eqn.zwanzig}
and the average is over all samples from the simulation performed with $H_0$.
...
This allows focusing on configurational partition function and express free energy estimator using potential energy $\Delta U(\vec{q})$
\begin{eqnarray}
\Delta F_{01} & = &  -\beta^{-1}\ln \expect{e^{-\Delta U(\vec{q})} }_0
\end{eqnarray}\label{eqn.zwanzig_configurational}
------
...
Jarzynski's equality relates the work ($W$) obtained from a one-directional non-equilibrium transition to the free energy difference between the end states:

\begin{eqnarray}
\langle e^{-\beta W} \rangle =e^{-\beta\Delta F}
\end{eqnarray}\label{eqn.jarzynski}

Expressing $\Delta F$ gives an estimator for the free energy difference, which is equivalent to Zwanzig's estimator when a transition is performed instantaneously:

\begin{eqnarray}
\Delta F = -\beta^{-1} \ln\langle e^{-\beta W} \rangle
\end{eqnarray}\label{eqn.jarzynski_estimator}

This estimator, however, is biased, and the bias depends strongly on the dissipated work $W_{dis}=W-\Delta F$.~\cite{gore2003bias}
The bias vanishes only for infinitely slow transitions where $\Delta F = \lim\limits_{\tau \to \infty} W$.
When work distributions are Gaussian, $p(W) \sim \mathcal{N}(\overline{W},\sigma^2)$, the Jarzynski estimator becomes:

\begin{eqnarray}
\Delta F = \overline{W} - \frac{\beta\sigma^2}{2}
\end{eqnarray}\label{eqn.jarzynski_gaussian}

The assumption of Gaussian work distributions usually holds only for slow switching that keeps the system close to equilibrium.~\cite{jarzynski1997nonequilibrium}
...
This estimate was first derived by Bennett and is hence generally called the Bennett Acceptance Ratio (BAR). It is solved by finding the free energy difference $\Delta F$ that satisfies the following implicit equation:

\begin{eqnarray}
 \sum_{k=1}^{n_i} \frac{1}{1 + \exp[\ln(\frac{n_i}{n_j}) + \beta \Delta U_{ij}(\vec{q_k}) - \beta \Delta F_{ij})
 ]} \nonumber \\
 =\sum_{l=1}^{n_j} \frac{1}{1 + \exp[\ln(\frac{n_j}{n_i}) - \beta \Delta U_{ij}(\vec{q_l}) + \beta \Delta F_{ij})]},
\end{eqnarray}\label{eq:bar}
...
Similarly to the analogy between Zwanzig's and Jarzynski's equations, Bennet Acceptance Ratio can be applied to estimate free energy differences from non-equilibrium transitions as well.
Replacing $U_{ij}(\vec{q})$ in equation~\ref{eq:bar} by work values $W_k$ and $W_l$ corresponding to the work monitored during the forward and reverse transitions, BAR can be written as follows~\cite{shirts2003equilibrium}:

\begin{eqnarray}
 \sum_{k=1}^{n_F} \frac{1}{1 + \exp[\ln(\frac{n_F}{n_R}) + \beta W_{k} - \beta\Delta F)]} \nonumber \\
 =\sum_{l=1}^{n_R} \frac{1}{1 + \exp[\ln(\frac{n_R}{n_F}) - \beta W_{l} + \beta\Delta F]},
\end{eqnarray}\label{eq:bar_noneq}
...
\end{eqnarray}\label{eq:cgi_noneq}
...
The free energy estimate can be viewed as an empirical average, like the one that arises in thermodynamic integration: if $\{Z_0, \dots, Z_{N-1}\}$ are samples drawn from an ergodic Markov process with stationary density $p(z)$ then for $N$ sufficiently large we expect 
\begin{equation}
    \int_\Omega f(z) p(z) \mathrm{d}z\approx \frac{1}{N}\sum_{i=0}^{N-1} f(Z_i), \label{eqn:empirical-average}
\end{equation}
where $\Omega$ denotes the state space and $f : \Omega \rightarrow \mathbb{R}$ is an observable. Even though not all estimators can be written in the exact form \eqref{eqn:empirical-average}, it will be sufficient to consider this case to get the main idea. The key object of study in this section is an estimator of 
\begin{equation}
    \mathrm{Var}_{Z_0 \sim \rho_0}\left(\frac{1}{N}\sum_{i=0}^{N-1} f(Z_i)\right)
\end{equation}
where $\rho_0(z)$ is the initial density for $Z_0$, that is, the probability distribution of starting configurations.
...
Consider a molecular dynamics trajectory started at some initial configuration $z_0 = (x_0, v_0)$, where $x_0 \in \mathbb{R}^{3 N}$ denotes the initial positions of the system and $v_0 \in \mathbb{R}^{3 N}$ its initial velocities, and denote the density at time $t$ by $\rho_t(z|z_0)$. This transition density is determined by the molecular dynamics integrator. The distribution of the velocities is usually ignored, since in the presence of a thermostat they are kept distributed roughly according to a Maxwell-Boltzmann distribution, so we only consider the $x$ component. In principle, one could consider the initial condition $x_0$ to be drawn from a more complicated initial distribution $\rho_0(x) \mathrm{d}x$, like a mixture of Dirac densities $\rho_0(x) \mathrm{d}x = \sum_{i = 1}^n \delta_{\{ x_i \}}(\mathrm{d}x) / n$, but any such "better" initial density would require some knowledge about the system at hand, such as the distribution of binding modes. 

It is difficult to make general statements about the behavior of $\rho_t (x | x_0)$. For systems not involving a protein or otherwise having a smaller number of degrees of freedom, it may be possible to identify clear "plateaus'' of convergence, i.e. long periods within which $\rho_t (x|x_0)$ does not does not vary much with the time $t$, at least in those reaction coordinates relevant to the free energy being estimated, and within which $\rho_t (x|x_0)$ is approximately invariant to $x_0$ in some appropriate starting domain. Consider, for instance, the estimation of free energies of hydration for small molecules ~\cite{paliwal2011benchmark}. In this case, the free energy estimates behave in accordance with classical statistical theory, and for sufficiently large $t$ and most $x_0$ the density $\rho_t (x | x_0)$ should not vary much along coordinates relevant to the free energy estimates. Quantifying ideas such as "long periods" or "vary much"  and of identifying relevant reaction coordinates is an active area of research and beyond the scope of this review.
...
What is certain is that one should not confuse take a long plateau in the free energy estimates as any evidence that the simulation has explored the entirety of the configuration space $\Omega \subset \mathbb{R}^{3N}$; most bio-molecular systems, even those involving only a single ligand, possess a whole hierarchy of timescales, the longest of which can easily exceed achievable simulation times. Consequently, the estimated error of the free energy estimate will typically be an underestimate of the true uncertainty.

% Of course, it _could_ be the case for some systems that \rho_t(x | x_0) is independent of x_0, and that the empirical distribution of any replicate is a good approximation for \rho_t (x|x_0). In general, however, we will only have access to a so-called quasi-stationary distribution; i.e. we will consider the distribution conditional on not having exceeded an escape time, and this distribution will only equilibrate in some degrees of freedom. 
...
\textbf{Definition}:
We define an \textbf{ensemble} to be a set of $R \geqslant 1$ trajectories, all started at the same initial configuration $x_0$, with initial velocity $v_0$ independently drawn from the Maxwell-Boltzmann distribution at the appropriate temperature and run for a chemical time $T$. We emphasize that, with this definition, all ensemble-based estimates are inherently tied to the choice of $x_0$; pathological choices  -- for example, a protein–ligand configuration that may quickly unbind -- can render the resulting estimates meaningless.

\subsubsection{Ensemble error estimates}

This section describes the simplest and, in many cases, the most robust procedure for estimating uncertainties in free energy calculations. For most practical applications, we recommend this approach when reporting statistical error bars. The procedure consists of simulating an ensemble $\{X^{(1)},\ldots,X^{(R)}\}$ of $R$ molecular dynamics trajectories initialized at $x_0$, and using the resulting free energy estimates $\{\Delta G^{(1)},\ldots,\Delta G^{(R)}\}$ to quantify uncertainty. The trajectories $\Delta
G^{(i)} (\cdot)$ are identically distributed and conditionally independent given $x_0$. For any time $T > 0$, the variance of any $\Delta G^{(i)} (T)$ can be estimated with the sample variance $S^2_{R} (T)$,
...
\end{equation}
In the notation of \eqref{eqn:empirical-average}, $Z_i = \Delta G^{(i)}(T)$; conditioned on $x_0$, the $Z_i$ are independent and identically distributed and are trivially a Markov process, with stationary distribution given by the distribution of $\Delta G^{(1)}(T)$. By direct calculation, we have (conditionally on $x_0$)
\begin{equation}
    \mathbb{E}_{x_0}[S_R^2(T)] = \mathrm{Var}_{x_0}(\Delta G^{(1)}(T))
\end{equation}
The above equation says that $S_R^2(T)$ is an unbiased estimator of the variance of $\Delta G^{(1)}(T)$, conditioned on $x_0$. By the Law of Large Numbers, as $R \rightarrow \infty$, we have $S^2_R (T) \rightarrow \mathrm{Var} _{x_0}(\Delta
G^{(1)} (T)) = \sigma^2 (T ; x_0)$. Furthermore, by conditional independence of the $\Delta G^{(i)}(T)$, for sufficiently large $R$ and any $T>0$ for which the variance is finite, we have
\begin{equation}\label{eqn:variance-of-empirical-mean}
\mathrm{Var}_{x_0}(\overline{\Delta G(T)}) = \frac{\mathrm{Var}_{x_0}(\Delta G^{(1)}(T))}{R} \approx \frac{S_R^2(T)}{R}
\end{equation}
...
\begin{equation}
  \mathrm{SE}_R (T ; x_0) = \sqrt{\frac{S_R^2(T)}{R}},
\end{equation}
which is the gold standard for reporting the error of a free energy estimate $\overline{\Delta G (T)} $ when a single initial condition $x_0$ is used. Note that $\mathrm{SE}_R
(T ; x_0) \rightarrow 0$ as $R \rightarrow \infty$ so long as $\mathrm{Var}_{x_0}(\Delta G^{(1)}(T))$ is finite, which is true if the $\Delta G^{(1)}(T)$ are bounded. Thus the variance can be made arbitrarily small simply by running enough replicates, but we note critically this does \textit{not} mean that we will have estimated the free energy to arbitrary precision: $\Delta G^{(1)}(T)$ in general will not be normally distributed, its value may vary greatly with both $T$ and $x_0$, and the variance may not be a good statistic to capture the error, as the distribution may be multi-modal, heavy-tailed and be sensitive to rare events. For example, one replicate out of many might explore a different mode of an important torsional angle, leading to a drastically different free energy estimate. 
%% MS: The rare event problem is a serious one, simulating even R = 100 replicates may not be "enough", depending on the purposes. Should we say this?
%% Should we also state the central limit theorem? 
In principle, the most robust error estimates can be obtained by starting from many different initial conditions $x_0, x_1, \dots$, and computing means and standard errors, but as mentioned in Section \ref{sec:timescales}, the value of this approach is limited by the user's ability to choose a suitable initial density $\rho_0(x)$.

\subsubsection{Per-replicate error estimates}

In the preceding case, we used minimal assumptions on the distribution of $\Delta G^{(i)}(T)$ to obtain estimates on $\sigma^2(T;x_0)$ by simulating conditionally independent replicates and using the sample variance \eqref{eq:sample-variance-definition}. To obtain per-replicate (i.e., per-$i$) error estimates, we impose much stricter assumptions on the distribution of $\Delta G^{(i)}(T)$,  namely that they satisfy a Central Limit Theorem (CLT) as $T\rightarrow \infty$, for each $i=1,\dots,R$:
$$ \sqrt{T} ( \Delta G^{(i)}(T) - \Delta G^\star) \Rightarrow N(0, \sigma^2_{\mathrm{asym}}(x_0))$$
where $\Delta G^\star$ denotes the true free energy difference, $\sigma_{\mathrm{asym}}^2(x_0)>0$ denotes the so-called "asymptotic variance", $ N(0, \sigma^2_{\mathrm{asym}}(x_0))$ is a Gaussian distribution with mean 0 and variance $\sigma_{\mathrm{asym}}^2(x_0)>0$, and $\Rightarrow$ denotes convergence in distribution. %% Should we define this? Might be a little too technical for the audience. Again, Durrett is one of the standard references. 
The CLT assumption implies that 
$$\sigma^2(T;x_0)\approx\frac{\sigma^2_{\mathrm{asym}}(x_0)}{T}.$$
In practice, it may not be the case that $\sigma^2(T;x_0)\propto T^{-1}$, nor even that $\sigma^2(T;x_0)$ is monotonically decreasing with $T$. In such a case, we are not in a regime where the CLT is satisfied and the remainder of this subsection does not apply.

To understand the difficulty of estimating $\mathrm{Var}_{x_0}(\Delta G^{(i)}(T))$ using only samples from replicate $i$, it is helpful to recall the empirical mean \eqref{eqn:empirical-average} as the canonical estimator. Suppose that $\{Z_i\}_{i\ge 0}$ is an ergodic Markov chain with stationary distribution $\pi$, that $Z_0 \sim \pi$, and that $t_{N-1}=T$. Let $f$ be a fixed observable. 
\begin{align}
\sigma^2(T;x_0) &= \mathrm{Var}_{\pi}\!\left(\frac{1}{N}\sum_{i=0}^{N-1} f(Z_i) \right) \\
&= \frac{1}{N^2}\sum_{i,j=0}^{N-1}
\mathrm{Cov}_{\pi}\!\bigl(f(Z_i), f(Z_j)\bigr) \\
&= \frac{1}{N^2}\left(
N\,\sigma_0^2
+ 2\sum_{i=1}^{N-1} (N-i)\,
\mathrm{Cov}_{\pi}\!\bigl(f(Z_0), f(Z_i)\bigr)
\right),
\label{eqn:variance-of-empirical-mean-long}
\end{align}
where
\[
\sigma_0^2 \equiv \mathrm{Var}_{Z\sim \pi}\!\bigl(f(Z)\bigr)
= \mathrm{Var}_{\pi}\!\bigl(f(Z_0)\bigr).
\]
The expression for the variance contains cross-terms which would vanish if $Z_0$ and $Z_i$ were independent; these additional cross terms add long-range temporal dependence, making the error estimation difficult. 

%% should we describe the atch means estiamtor? maybe not...
For the remainder of the section we focus on a single replicate. The simplest generalization of \eqref{eq:sample-variance-definition} to the case of time-correlated samples is the non-overlapping batch means estimator. Let $\{t_0,\dots,t_{N-1}\}$ be a discretization of $[0,T]$ with $t_0=0$ and $t_{N-1}=T$;  we discuss the choices of discretization width $t_{i+1} - t_i$ and initial time $t_0$ in later subsections. 

Partition the $N$ sequential samples of this single replicate into $B$ consecutive batches of equal size $M$, so that $N = BM$. Batch $k$ consists of the samples $\{X(t_{kM}),\dots,X(t_{(k+1)M-1})\}, k=0,\dots,B-1$, with $X(t_0) = x_0$. Let
\[
\Delta G_k = \Delta G(t_{kM}:t_{(k+1)M})
\]
denote the estimator obtained using only the samples in batch $k$, and define the batch mean
\[
\overline{\Delta G} \equiv \frac{1}{B}\sum_{k=0}^{B-1} \Delta G_k .
\]
The non-overlapping batch means estimator is then
\[
\hat\sigma_B^2(T;x_0)
= \frac{1}{B(B-1)}\sum_{k=0}^{B-1}
\left(\Delta G_k - \overline{\Delta G}\right)^2 .
\]
This estimator may look complicated, but it is just the application of the sample variance to the estimates on  $B$ non-overlapping time blocks of length $M$, which gives the variance using $M$ samples. The sample variance across $\Delta G_k$ estimates $\mathrm{Var}_{x_0}(\Delta G_k)$, and the additional factor of $1/B$ yields an estimate of $\mathrm{Var}_{x_0}(\overline{\Delta G})$.

If the final time $T=t_{N-1}$ and the block length $M$ are well-chosen, then with enough replicates $R\geqslant 1$, we should find that
$$ \hat\sigma_B^2(T; x_0) \approx S_R^2(T ; x_0) .$$
...
\begin{equation}
C_i \equiv
\frac{\mathrm{Cov}_{\pi}\!\bigl(f(Z_0), f(Z_i)\bigr)}{\sigma_0^2}.
...
\begin{equation}
\mathrm{Var}_{\pi}\!\left(\frac{1}{N}\sum_{i=0}^{N-1} f(Z_i) \right)
=
\frac{\sigma_0^2}{N}
\left(
...
\begin{eqnarray}
g &\equiv& 1 + 2 \tau_\mathrm{eq} \approx  \frac{\sigma^2(T;x_0)}{\sigma_0^2} N \label{eq:statistical-inefficiency-definition} 
\end{eqnarray}
is referred to as the \textit{statistical inefficiency}. Loosely speaking, $g$ measures the number of indices $i$ between roughly independent samples, and is therefore sometimes referred to as the statistical inefficiency. The role of estimators such as non-overlapping batch means is precisely to estimate $\sigma^2(T;x_0)$ directly from correlated data, which is roughly equivalent to estimating $g$. 
...
In the preceding section we considered a discretization   $\{t_0, \dots, t_{N-1}\}$ of $[0,T]$, so that $t_0=0, t_{N-1}=T$ . In this section we briefly explain how the choice of discretization affects the estimates of the free energy and that of its error. 
...
A related practical question concerns how a fixed computational budget should be allocated between the number of independent replicates $R$ and the length of each trajectory $T$. This question is inherently problem dependent, as it presumes some knowledge of how statistical error evolves with simulation time. 

Let $\Delta G^{(i)}(t)$ denote the free energy estimate obtained from replicate $i$ after simulation time $t$, and suppose that, for fixed $t$, the conditional variance 
\[
E(t)^2 \;\equiv\; \mathrm{Var}_{x_0}\!\bigl(\Delta G^{(i)}(t)\bigr)
\]
is finite. Given a fixed computational budget $B$, we may consider simple allocation schemes in which $B = RT$, where $R$ replicates are each run for time $T$. Under this constraint, the conditional error scales as
\[
\frac{E(T)}{\sqrt{R}}
= \frac{E(T)}{\sqrt{B/T}}
= \frac{\sqrt{T}\,E(T)}{\sqrt{B}}
\;\equiv\; \frac{J(T)}{\sqrt{B}}.
\]
The function $J(T)$ thus quantifies the tradeoff between trajectory length and the number of replicates. In practice, one might observe that $J(T)$ is minimized at very small values of $T$. If $T$ is very small -- say, less than 1 ns -- the finding is questionable and suggests the error is not being properly estimated. 


% While one might hope for the asymptotic scaling $E(t)\sim t^{-1/2}$, such behavior is rarely observed at practically accessible timescales. Nevertheless, conditioned on their initial states, the estimates $\{\Delta G^{(i)}(t)\}_{i=1}^R$ obtained from different replicates are conditionally independent and identically distributed. Consequently, the conditional variance of the averaged estimator
% \[
% \frac{1}{R}\sum_{i=1}^R \Delta G^{(i)}(t)
% \]
% scales as $E(t)^2/R$, and the corresponding conditional error scales as $E(t)/\sqrt{R}$.
...
Additionally, as discussed in Section~\ref{subsec:net charge}, changes in formal charge are still problematic and require additional care in their treatment. In many implementations, the approaches for handling such perturbations are not yet robustly implemented, and even when they are, formal charge perturbations typically introduce additional uncertainties.
...
We hope that the best practice guide provides a set of tools that allow a better understanding of how to setup, run, and reliably interpret single alchemical free energy calculations. The best practices for running alchemical free energy calculations at large scale are discussed in an upcoming companion article~\cite{mey_largescale_inprep}.
...
\item [] \textbf{Simulation software: Free/low cost academic and Commercial}
	\begin{itemize}
    \item \href{https://ambermd.org/}{AMBER} is a software suite~\cite{Case_JChemInfModel_2025_v65_p7835} that consists of a GPU-accelerated free energy engine (pmemd.cuda, free for academics)~\cite{Lee_JChemTheoryComput_2017_v13_p3077,Giese_JChemTheoryComput_2018_v14_p1564,Lee_JChemInfModel_2018_v58_p2043} that supports a wide range of alchemical free energy methods~\cite{tsai2023amber,lee2023aces,zhang_jchemtheorycomput_2024_v20_p3935}, and AmberTools~\cite{case_jcheminfmodel_2023_v63_p6183}, a free and open-source collection of programs used to set up, run, and analyze molecular simulations.
...
\begin{itemize}
    ...
    \item \href{https://github.com/OpenBioSim/ghostly}{Ghostly} is as a standalone tool to programmatically implement the dummy atom modifications discussed in~\citeauthor{Fleck_2021}~\cite{Fleck_2021} for a range of file formats~\cite{hedges_ghostly_2025.2.0}.
    ...
    \item \href{http://www.ties-service.org}{TIES} setting up, running, and analyzing alchemical free energy simulations using NAMD and OpenMM \url{https://github.com/UCL-CCS/TIES_MD}~\cite{bieniek2023}.
...
\item []\textbf{Analysis tools:}
    ...
    \item \href{https://gitlab.com/RutgersLBSR/fe-toolkit}{FE-Toolkit}: is a collection of packages for free energy analysis, including network-wide MBAR analysis~\cite{giese2025fe} with cycle closure and (optionally) experimental constraints, and ND free energy surface~\cite{Giese_JPhysChemA_2021_v125_p4216} and surface accelerated string~\cite{Giese_JChemTheoryComput_2024_v20_p2058} methods.
	\end{itemize}
...
\begin{itemize}
    ...
    \item Does my system contain any structures that require custom force field parameters?
    \item What simulation protocol will provide the most evidence to verify my hypothesis?
    \item What simulation protocols are suitable for my system? (For example, is there a net-charge change requiring e.g., a co-alchemical ion approach? Are these ligands with different binding poses which are unsuitable for a traditional relative binding free energy calculation?)
...
\begin{itemize}
    \item Prepare your system consistent with your understanding of the biology, chemistry, and physics of the system, for example:
    \begin{itemize}
        \item Check that your ligands have the biologically correct binding pose(s)
        \item Check that your system has the correct protonation state at assay pH
        \item Prepare a realistic membrane environment if required
    \end{itemize}
...
\begin{itemize}
    ...
    \item Apply the appropriate standard state terms
    \item Apply the appropriate correction terms (e.g., for multiple chemical states)
...
\begin{checklist}{How do I know which simulations are unreliable?}
    ...
    \item If you observe hysteresis in perturbations
    \item Energy differences \textgreater$\sim$15 kcal$\cdot$mol$^{-1}$  are likely unreliable
    \newline\newline\textit{Absolute:}
    \item Free energies differences of binding of \textless$\sim$-15 kcal$\cdot$mol$^{-1}$  are likely unreliable
    \item The ligand has not sampled most of the intended region after the decoupling step
    \item The ligand drifts out of the binding site at any $\lambda$ state
        \end{itemize}
\end{checklist}
...
\begin{itemize}
    \item Decide which calculated values to assess, e.g., shifted or unshifted $\Delta G$,  $\Delta \Delta G$ of computed edges.
...
%%%%%%%%%%%%%%%%
\textbf{ASJSM}: Coordinated the document, contributed to most sections, and co-designed Figs.~\ref{fig:fig_binding_thermodynamic_cycle},~\ref{fig:fig_topology},~\ref{fig:fig_mcss},~\ref{fig:fig_absolute_thermodynamic_cycle}, and created Figs.~\ref{fig:fig_what_is_lambda},~\ref{fig:fig_what_is_alchemy},~\ref{fig:overlap},~\ref{fig:pmf} and replotted~\ref{fig:automatic-equilibration-detection}.\\
...
\textbf{HBM} Contributed to Sec.~\ref{subsec:binding}, reviewed updates to the discussion of uncertainty quantification throughout the paper, and helped edit the paper.\\
\textbf{MC} Contributed to ``Changes in net charge can be challenging/problematic'' in Sec. \ref{subsec:net charge}. \\
\textbf{JDC}: Wrote Sec.~\ref{sec:automatic-equilibration-detection} discussed structure and design of the whole document, suggested Figs.~\ref{fig:fig_what_is_alchemy} and~\ref{fig:fig_sampling_scheme}. \\
...
\textbf{VG} Updated the discussion of non-equilibrium switching throughout the article, and reviewed Sec.~\ref{subsec:net charge}.\\
\textbf{DFH} Contributed to Sec.~\ref{sec:software}, reviewed the discussion of non-equilibrium switching, and helped edit the paper.\\
...
\textbf{MK}: Contributed to Sec.~\ref{sec:data_analysis}, provided the data for figure~\ref{fig:overlap}, and helped edit the paper.\\
...
\textbf{JS}: Created Figs.~ \ref{fig:fig_what_is_alchemy},~\ref{fig:fig_binding_thermodynamic_cycle},~\ref{fig:fig_topology},~\ref{fig:fig_mcss}, and ~\ref{fig:fig_absolute_thermodynamic_cycle}. Wrote the checklist Sec.~\ref{sec:checklist}, and contributed to general formatting discussions and editing.\\
...
\textbf{ST}: Created Fig.~\ref{fig:fig_topology} (based on an earlier version by JS).\\
\textbf{GT}: Contributed to Sec.~\ref{sec:intro} and helped edit the paper.\\
\textbf{HX}: Contributed to Sec.~\ref{sec:relative-fe-protocol}, and to Sec.~\ref{sec:are-they-good}.\\
\textbf{MS:} Proof read some of the mathematical material in the section on uncertainty quantification and added some details. \\
\textbf{DMY:} Contributed to the discussion of AMBER, AmberTools and FE-ToolKit, softcore potentials, ACES enhanced sampling and CBFE methods, and topologies in Sec.~\ref{sec:relative-fe-protocol}, reviewed the updates to Sec.~\ref{sec:theory}, and made minor comments and additions throughout. \\
...
We thank Lester O. Hedges for clarifying details of the RBFE implementation in BioSimSpace, and we thank Mateusz K. Bieniek for explaining details of TIES.
...
MS is currently employed by D. E. Shaw Research. Any contributions do not represent those of the company. 
...
CO and MC acknowledge the financial support by the Austrian Federal Ministry for Digital and Economic Affairs, the National Foundation for Research, Technology and Development, the Christian Doppler Research Association, BASF, and Boehringer Ingelheim.
