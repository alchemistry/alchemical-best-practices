...
Once a single coordinate set approach is selected is selected, a crucial next step is to identify the common atoms which will not be perturbed between the two molecules (respectively end-states).
...
In general, there are many ways on how an MCSS can be calculated. One of the most prominent approaches is to estimate the MCSS with a 2D subgraph isomorphism solver(SIS) using the 2D topological stuructre information of molecules\cite{raymond2002maximum}.
