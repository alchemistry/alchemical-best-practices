...
\egnote{Is this section necessary? We could add a comment regarding weak binders in the preceding section clarifying how the theory covers weak binders (as well as non-binders) and the precautions the user should take to obtain accurate estimates (relative to the experiments).}
...
In absolute calculations, the need to explicitly define a binding site applies to the fully interacting state in the complex leg of the thermodynamic cycle (top-right state in Fig.~\ref{fig:fig_absolute_thermodynamic_cycle}),  while in relative calculations the binding site must be defined at both end states of the complex leg (top- and bottom-right states in Fig.~\ref{fig:fig_binding_thermodynamic_cycle}). \egnote{I do not agree with the latter. Binding site restraints apply to the coupled and uncoupled states. The same is true for RBFE for both ligands. (see above)}
...
In the case of weak binders, \egnote{I would say for all ligands: strong, weak, as well as non-binders.} using a flat-bottom or harmonic restraint \egnote{Harmonic restraints are not suitable as binding site restraints, see above} between receptor and ligand in the bound state(s) can prevent dissociations~\cite{georgiou2017pushing,rizzi2019sampl6}.
We stress that this type of restraint is normally avoided as it generally introduces bias in the free energy estimate, which is why the restraint is usually activated only in the intermediate states in absolute calculations. \egnote{The latter statement is incorrect. Binding site restraints are always necessary in principle. }
The bias can be corrected through reweighting schemes~\cite{rizzi2019sampl6}, \egnote{Auxiliary restraints and biasing potentials require reweighting. But binding site restraints do not generally require any action in post-processing except for adding the $C^\circ V_{\rm site}$ term. } but this post-processing step can be avoided if a flat-bottom restraint is used and the ligand is never sampled while hitting the potential wall during the simulation in the bound state as the numerical correction will be exactly zero. \egnote{The ligand frequently hitting the boundary of the flat-bottom restraints while in the coupled state is indicative of an inappropriate definition of the binding site: the ligand is attempting to access a lower free energy binding pose. The free energy estimate is unlikely to agree with experiments but the calculation is otherwise valid. It reports the binding free energy in the binding pose specified by the region allowed by the flat-bottom binding site restraint.}
...
Again, this step is particularly important for weak binders \egnote{but necessary, in principle, for all binders} as their free energy of binding is known to be more sensitive to the definition of the binding site~\cite{gilson1997statisticalthermodynamic}.
...
In the latter case, dissociations can be averted simply by increasing the spring constant and/or reducing the radius of the restraint potential to prevent the exploration of ligand conformations outside the binding site in the decoupled state (bottom-right state in Fig.~\ref{fig:fig_absolute_thermodynamic_cycle}) that could be propagated to the bound state. \egnote{I disagree. Binding site restraints are never turned off, especially at the uncoupled state, which would result in the ligand wandering away from the receptor. Additional restraints can be applied to reduce the extent of conformational sampling in the intermediate alchemical steps. Conversely, energy barrier-flattening potentials can be applied to accelerate conformational sampling. These auxiliary potentials must be turned on and then turned off so that they are not active at the endstates. Alternatively, thermodynamic reweighting must be applied in post-processing to remove their effects on the binding free energy estimate. }
