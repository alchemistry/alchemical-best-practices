...
In this section, we explore the various possible types of error analysis that can be performed, depending on the behavior of $\rho_t$. We begin with a definition.
...
\end{equation}
The above equation says that $S_R^2(t)$ is an unbiased estimator of the variance. By a law of large numbers, as $R \rightarrow \infty$, we have $S^2_R (t) \rightarrow \mathrm{Var} _{x_0}(\Delta
G^{(1)} (t)) = \sigma^2 (t ; x_0)$. 
...
In the preceding case, we used minimal assumptions on the distribution of $\Delta G^{(i)}(t)$ to obtain estimates on $\sigma^2(t;x_0)$ by simulating conditionally independent replicates and using the sample variance. To obtain per-replicate error estimates, we impose much stricter assumptions on the distribution of $\Delta G^{(i)}(t)$, namely that they satisfy a Central Limit Theorem (CLT) as $t\rightarrow \infty$:
...
In practice, it may not be the case that $\sigma^2(t;x_0)\propto t^{-1}$, nor even that $\sigma^2(t;x_0)$ is monotonically decreasing with $t>0$. In such a case, we are not in a regime where the CLT is satisfied and the remainder of this subsection does not apply.

The simplest generalization of \eqref{eq:sample-variance-definition} to the case of time-correlated samples is the non-overlapping batch means estimator.  Consider any discretization  $\{t_0, \dots, t_{N-1}\}$ of $[0,T]$, so that $t_0=0, t_{N-1}=T$ -- we relegate the choice of discretization width $t_{i+1} - t_i$ and initial time $t_0$ to the following section -- and partition the $N$ times into $B$ batches of length $M$, so that $N = BM$. Let $\Delta G(t_i : t_{i+M})$ denote the the estimate obtained using only the samples $X(t_i),\dots, X(t_{i+M-1})$. The non-overlapping batch means estimator is then

$$ \hat\sigma^2_B(T ; x_0) = \frac{1}{B(B -1)}\sum_{k=0}^{B -1} \left(\Delta G(t_{k M} : t_{(k+1)M}) - \frac{1}{B} \sum_{k=0}^{B-1} \Delta G(t_{kM} : t_{(k+1)M})\right)^2$$
...
It is also useful to note that the variance $\sigma^2(t;x_0)$ can be written as
$$\sigma^2(t;x_0) =  \sigma_0^2(t;x_0) \tau_{\mathrm{int}},$$
where $\tau_{\mathrm{int}}$ is the so-called integrated autocorrelation time of the free energy estimate. Loosely speaking, $\tau_{\mathrm{int}}$ is the number of time indices required to obtain an "independent" sample. It is also sometimes referred to as the statistical inefficiency.
...
\subsubsection{Per-replicate errors and subsampling}

% How should one sub-sample? I believe this section can be short:

- One is always de-facto sub-sampling, unless a sample is stored every molecular dynamics timestep. This is typically infeasible.
- Sub-sampling the actual saved trajectory is never necessary, especially not for single trajectories
- If the statistical inefficiency (aka integrated autocorrelation time) is (say) 100, subsampling every 10 steps is probably not going to hurt much; could be useful for slow programs
- Computing error bars using multiple trajectories is sometimes simplified by subsampling


\subsubsection{{Per-replicate errors and burn-in times}}
