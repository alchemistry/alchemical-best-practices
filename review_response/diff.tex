...
%DIF DEL manuscript.tex        Fri Oct 16 18:14:19 2020
%DIF ADD manuscript_v1.0.tex   Tue Nov  3 19:03:31 2020
...
%DIF -------
\newcommand{\githubrepository}{\url{https://github.com/alchemistry/alchemical-best-practices}} %this should be the main github repository for this article %DIF > 
%DIF -------
...
%DIF 39-41c39-42
%DIF < \author[7]{Bryce K. Allen}
%DIF < \author[2]{Hannah E. Bruce Macdonald}
%DIF < \author[2*]{John D. Chodera}
%DIF -------
\author[2]{Bryce K. Allen} %DIF > 
\author[3]{Hannah E. Bruce Macdonald} %DIF > 
\author[3*]{John D. Chodera} %DIF > 
\author[9]{David F. Hahn} %DIF > 
%DIF -------
\author[1,10]{Maximilian Kuhn}
\author[1]{Julien Michel}
%DIF 44-47c45-48
%DIF < \author[3*]{David L. Mobley}
%DIF < \author[11]{Levi N. Naden}
%DIF < \author[4]{Samarjeet Prasad}
%DIF < \author[2,8]{Andrea Rizzi}
%DIF -------
\author[4*]{David L. Mobley} %DIF > 
\author[5]{Levi N. Naden} %DIF > 
\author[6]{Samarjeet Prasad} %DIF > 
\author[2,7]{Andrea Rizzi} %DIF > 
%DIF -------
\author[1]{Jenke Scheen}
%DIF 49c50
%DIF < \author[6*]{Michael R. Shirts}
%DIF -------
\author[8*]{Michael R. Shirts} %DIF > 
%DIF -------
\author[9]{Gary Tresadern}
%DIF 51c52
%DIF < \author[7]{Huafeng Xu}
%DIF -------
\author[2]{Huafeng Xu} %DIF > 
%DIF -------
%
\affil[1]{EaStCHEM School of Chemistry, David Brewster Road, Joseph Black Building, The King's Buildings, Edinburgh, EH9 3FJ, UK}
%DIF 54-59c55-61
%DIF < \affil[2]{Computational and Systems Biology Program, Sloan Kettering Institute, Memorial Sloan Kettering Cancer Center, New York NY, USA}
%DIF < \affil[3]{Departments of Pharmaceutical Sciences and Chemistry, University of California, Irvine, USA}
%DIF < \affil[4]{National Institutes of Health, Bethesda, MD, USA}
%DIF < \affil[6]{University of Colorado Boulder, Boulder, CO, USA}
%DIF < \affil[7]{Silicon Therapeutics, Boston, MA, USA}
%DIF < \affil[8]{Tri-Institutional Training Program in Computational Biology and Medicine, New York, NY, USA}
%DIF -------
\affil[2]{Silicon Therapeutics, Boston, MA, USA} %DIF > 
\affil[3]{Computational and Systems Biology Program, Sloan Kettering Institute, Memorial Sloan Kettering Cancer Center, New York NY, USA} %DIF > 
\affil[4]{Departments of Pharmaceutical Sciences and Chemistry, University of California, Irvine, USA} %DIF > 
\affil[5]{Molecular Sciences Software Institute, Blacksburg VA, USA} %DIF > 
\affil[6]{National Institutes of Health, Bethesda, MD, USA} %DIF > 
\affil[7]{Tri-Institutional Training Program in Computational Biology and Medicine, New York, NY, USA} %DIF > 
\affil[8]{University of Colorado Boulder, Boulder, CO, USA} %DIF > 
%DIF -------
...
%DIF 62c64
%DIF < \affil[11]{Molecular Sciences Software Institute, Blacksburg VA, USA}
%DIF -------
 %DIF > 
%DIF -------
%
...
%DIF 83a85
\orcid{David F. Hahn}{0000-0003-2830-6880} %DIF > 
%DIF -------
...
In this paper, we review current best practices for several popular application domains of \DIFaddbegin \DIFadd{equilibrium }\DIFaddend alchemical free energy calculations, including relative and absolute small molecule binding free energy calculations to biomolecular targets.
...
Subsequent work in the 2000s led to improved implementations of alchemical methods in popular biomolecular simulation packages~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{shirts2003extremely,shirts2005solvation,vanderspoel2005gromacs, mermelstein2018fast, wang2015accurate, hedges2019biosimspace}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{shirts2003extremely,shirts2005solvation,vanderspoel2005gromacs, mermelstein2018fast, wang2015accurate, hedges2019biosimspace, riniker2011calculation}
}%DIFAUXCMD
}\DIFaddend . 
...
Particular focus will be given to aspects of the molecular simulations which are unique to alchemical calculations---these include the calculation of transfer free energies (hydration free energies, partition coefficients, etc.), and binding free energies (absolute and relative). \DIFaddbegin \DIFadd{We primarily focus on }\emph{\DIFadd{equilibrium}} \DIFadd{free energy calculations in this best practices guide, as best practices for these are more developed, and nonequilibrium techniques may warrant their own guide as such practices evolve. 
}\DIFaddend 
...
\DIFaddbegin \item \DIFadd{nonequilibrium free energy calculations~\mbox{%DIFAUXCMD
\cite{gapsys2020large}
}%DIFAUXCMD
}\item \DIFadd{Free energy calculations using QM/MM methods~\mbox{%DIFAUXCMD
\cite{beierlein2011simple,dybeck2016comparison,cave-ayland2015direct}
}%DIFAUXCMD
.
}\item \DIFadd{Free energy calculations using Machine learning methods~\mbox{%DIFAUXCMD
\cite{rufa2020chemical, scheen2020hybrid, cole2020machine}
}%DIFAUXCMD
...
\begin{tcolorbox}[title=Acronyms, colback=blue!10!white]
    {\bf CPU} --- Central Processing Unit\\
     ...
     {\bf MUE} --- Mean \DIFdelbegin \DIFdel{unsigned error}\DIFdelend \DIFaddbegin \DIFadd{Unsigned Error}\DIFaddend \\
...
$\Delta G$ --- Gibbs free energy (isothermal isobaric ensemble)\DIFaddbegin \DIFadd{, Gibbs function, or free enthalpy, though the most common term Gibbs free energy is used in the text}\DIFaddend \\
$\Delta A$ --- Helmholtz free energy (canonical ensemble) \DIFaddbegin \DIFadd{or Helmholtz function, with Helmholtz free energy used in the text.}\DIFaddend \\
$\Delta f$ --- reduced (dimensionless) free energy \\
$\Delta \hat{f}$ --- estimate from an estimator for the reduced free energy\\
$\Gamma$ --- \DIFdelbegin \DIFdel{conformation }\DIFdelend \DIFaddbegin \DIFadd{configurational }\DIFaddend space accessible by simulations \\
...
\begin{equation}\DIFaddbegin \label{eq:law-mass-action}
 \DIFaddend K_b^{\circ} = c^{\circ}\frac{[RL]}{[L][R]}.
\end{equation}
The standard state concentration $c^\circ$ depends on the reference state, but it is usually set to $1\,\mathrm{mol}/\mathrm{L}$ assuming a constant pressure of $1\,\mathrm{atm}$ (see also Sec.~\ref{sec:standardstate-restraints})\DIFaddbegin \DIFadd{, but also holds for dilute solutions if thermodynamic activities can be approximated by concentrations~\mbox{%DIFAUXCMD
\cite{gilson1997statisticalthermodynamic}
}%DIFAUXCMD
}\DIFaddend .
Thus, the \DIFaddbegin \DIFadd{standard }\DIFaddend Gibbs free energy of binding $\Delta G_{\mathrm{bind}}$ is given by:
\begin{equation}
    \Delta G_{\mathrm{bind},L} = -k_BT\ln K_b^{\circ},
\end{equation}
where $k_B$ is the Boltzmann constant and $T$ the temperature of the system.
\DIFaddbegin \DIFadd{Note that, unless otherwise specified, we use $\Delta G$ throughout the paper to refer to the }\textit{\DIFadd{standard}} \DIFadd{free energy of binding (or solvation) (see also Sec.~\ref{sec:standardstate-restraints}), which is often indicated in other works with $\Delta G^{\circ}$ to differentiate it from non-standard free energies.
...
\DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{The law of mass action in Eq.~\ref{eq:law-mass-action} is not directly applicable to typical molecular simulations as they normally include a single receptor/ligand in a small box (i.e., using large concentrations)~\mbox{%DIFAUXCMD
\cite{jong2011determining}
}%DIFAUXCMD
.
Instead, a }\DIFaddend natural, though generally very computationally expensive, way to estimate the equilibrium constant is by directly simulating several binding and unbinding events and computing the probability of finding the receptor-ligand system in the bound state, $P(RL)$, or the unbound state, $P(R+L)$.
...
Let $\Gamma_{\mathrm{bound}}$ and $\Gamma_{\mathrm{unbound}}$ be the set of receptor-ligand \DIFdelbegin \DIFdel{conformations }\DIFdelend \DIFaddbegin \DIFadd{configurations }\DIFaddend $\vec{q}$ that we consider bound and unbound respectively.
The probability of a \DIFdelbegin \DIFdel{conformation }\DIFdelend \DIFaddbegin \DIFadd{configuration }\DIFaddend $\vec{q}$ is given by the Boltzmann probability density function
\begin{equation}
    P(\vec{q}) = \DIFdelbegin \DIFdel{\frac{\exp\left( \beta U(\vec{q}) \right)}{\int_{\Gamma} \exp\left( \beta U(\vec{q}) \right) \, d\vec{q}} }\DIFdelend \DIFaddbegin \DIFadd{\frac{\exp\left( -\beta U(\vec{q}) \right)}{\int_{\Gamma} \exp\left( -\beta U(\vec{q}) \right) \, d\vec{q}} }\DIFaddend \, ,
    \label{eq:conf_probability}
\end{equation}
where $\beta = (k_B T)^{-1}$ is the inverse temperature, $U(\vec{q})$ is the potential energy of \DIFdelbegin \DIFdel{conformation }\DIFdelend \DIFaddbegin \DIFadd{configuration }\DIFaddend $\vec{q}$, and the integration is over the set of all possible \DIFdelbegin \DIFdel{conformations }\DIFdelend \DIFaddbegin \DIFadd{configurations }\DIFaddend accessible in the simulation box volume $\Gamma$, with $\Gamma_{\mathrm{bound}},\Gamma_{\mathrm{unbound}} \subset \Gamma$.
If the simulation is long enough, we expect the fraction of \DIFdelbegin \DIFdel{conformations }\DIFdelend \DIFaddbegin \DIFadd{configurations }\DIFaddend $\vec{q}$ found in the bound state to converge to
\begin{equation}
    P(RL) = \int_{\Gamma_{\mathrm{bound}}} P(\vec{q}) \, d\vec{q} = \DIFdelbegin \DIFdel{\frac{\int_{\Gamma_{\mathrm{bound}}} \exp\left( \beta U(\vec{q}) \right) \, d\vec{q}}{\int_{\Gamma} \exp\left( \beta U(\vec{q}) \right) \, d\vec{q}} }\DIFdelend \DIFaddbegin \DIFadd{\frac{\int_{\Gamma_{\mathrm{bound}}} \exp\left( -\beta U(\vec{q}) \right) \, d\vec{q}}{\int_{\Gamma} \exp\left( -\beta U(\vec{q}) \right) \, d\vec{q}} }\DIFaddend \, .
\end{equation}
...
In many cases, the quantity of interest is the change in binding affinity between a compound $A$ and a related compound $B$ (e.g., by modifying one \DIFaddbegin \DIFadd{of }\DIFaddend the drug scaffold's substituents, see (Fig.~\ref{fig:fig_what_is_alchemy} \textbf{F}), which, by using Eq.~\ref{eq:binding-free-energy-from-bound-unbound-probability-ratio} and \ref{eq:bound-unbound-probability-ratio} is given by
...
Note that, when $A$ and $B$ have different number of atoms, \DIFaddbegin \DIFadd{the factors $ \ln \frac{Z(RB)}{Z(RA)}$ and $\ln \frac{Z(B)}{Z(A)}$ in   }\DIFaddend Eq.~\ref{eq:delta-delta-G-alchemical} \DIFdelbegin \DIFdel{implies the presence of a factor having }\DIFdelend \DIFaddbegin \DIFadd{appear both to have factors with }\DIFaddend units of volume \DIFdelbegin \DIFdel{entering both logarithms, which require unitless arguments.
However, the value of this factor is inconsequential as it cancels out in practice}\DIFdelend \DIFaddbegin \DIFadd{in the logarithms, but these factors exactly cancel between the terms}\DIFaddend .
...
\begin{figure}
    ...
    The relative binding free energy difference between two small molecules, $\Delta \Delta G_{\mathrm{bind}, A \rightarrow B} \equiv \Delta G_{\mathrm{bind}, B} - \Delta G_{\mathrm{bind}, A}$---here benzyl alcohol (top) to toluene (bottom)---can be computed as a difference between two alchemical transformations, $\Delta G_\mathrm{bound} - \Delta G_\mathrm{solvated}$, where $\Delta G_\mathrm{bound}$ represents the free energy change of transforming $A \rightarrow B$ in complex, i.e. bound to a host molecule, and \DIFdelbeginFL \DIFdelFL{$\Delta G_\mathrm{solvated}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\Delta G_\mathrm{unbound}$ }\DIFaddendFL the free energy change of transforming $A \rightarrow B$ in solvent, typically water.}
...
Roughly, the phase-space overlap between two thermodynamic states measures the degree to which high-probability \DIFdelbegin \DIFdel{conformations }\DIFdelend \DIFaddbegin \DIFadd{configurations }\DIFaddend (i.e., those with very negative potential energy) in one state are also high-probability \DIFdelbegin \DIFdel{conformations }\DIFdelend \DIFaddbegin \DIFadd{configurations }\DIFaddend in the other state (see Sec.~\ref{sec:are-they-good} and Fig.~\ref{fig:fig_what_is_lambda}).

\DIFdelbegin \DIFdel{To }\DIFdelend \DIFaddbegin \DIFadd{Equilibrium free energy calculations, our focus here, }\DIFaddend solve the problem of having poor overlap between the states of interest, multiple intermediate alchemical states are introduced at values $\vec{\lambda}_A = \vec{\lambda}_0, \vec{\lambda}_1, \cdots, \vec{\lambda}_K = \vec{\lambda}_B$ so that each pair of consecutive states $\vec{\lambda}_k, \vec{\lambda}_{k+1}$ share good overlap.
Each intermediate state models a ligand that is neither $A$ nor $B$ but a mix of the two.
Many estimators (e.g., exponential reweighting (EXP)~\cite{zwanzig1954hightemperature} and \DIFdelbegin \DIFdel{Bennet}\DIFdelend \DIFaddbegin \DIFadd{Bennett}\DIFaddend 's acceptance ratio (BAR)~\cite{bennett1976efficient,shirts2003equilibriuma}) can then be used to compute the free energy as
\begin{equation}
...
\end{equation}
While this strategy usually results in sampling thermodynamic states whose Boltzmann distributions are very similar, thus collecting information that is to some degree redundant, some estimators such as the Multistate Bennett acceptance ratio (MBAR)~\cite{shirts2008statisticallya} can exploit similarities between states to improve the precision of the estimates. This is achieved by using the \DIFdelbegin \DIFdel{conformations }\DIFdelend \DIFaddbegin \DIFadd{configurations }\DIFaddend sampled at all alchemical states $\{\vec{\lambda}_k \}$ to compute the free energy difference $\Delta f(\vec{\lambda}_i, \vec{\lambda}_{j})$ between any pair of states $i,j$ (see Sec.~\ref{subsec:estimators}).

\DIFaddbegin \DIFadd{Nonequilibrium free energy techniques provide an alternate approach to this problem, driving $\lambda$ between states, but these are not our focus here.~\mbox{%DIFAUXCMD
\cite{jarzynski1997nonequilibrium,jarzynski1998equilibrium,crooks2000pathensemble, gapsys2020large}
}%DIFAUXCMD
}

\DIFaddend \paragraph{How do absolute free energy calculations differ from relative?}
...
\DIFdelbegin \DIFdel{Current }\DIFdelend \DIFaddbegin \DIFadd{We first note that the accuracy of any free energy calculation method will be dependent on the quality of the underlying force field.  Therefore, any description of the force field for the molecules under study must be carefully checked to be sufficiently accurate to experiment. In particular, one must make sure that if using automatically generated molecular descriptions from either one's own workflow or some other computational chemistry program, there are not obvious problems in these files either through errors in the workflow, or lack of chemical coverage in the data used to construct the molecular description. Torsional parameters, in particular can be misassigned or improperly parameterized. 
}

\DIFadd{Given reasonably accurate, current generation force fields correctly implemented, }\DIFaddend alchemical free energy calculations involving small molecules seem to achieve, in favorable cases, root mean square (RMS) errors \DIFaddbegin \DIFadd{relative to experiment }\DIFaddend around 1-2 kcal/mol depending on force field, system, and a variety of other factors such as simulation time, sampling method, and whether the calculations employed are absolute or relative. A small selection of example datasets and case studies can be found in Sec.~\ref{sec:benchmark} at the end of this document.
...
Furthermore, perturbations for the same chemotype in different pockets of the BACE enzyme gave varied errors~\cite{keranen2017acylguanidine}. Here the errors refer to the difference in $\Delta G$ derived \DIFdelbegin \DIFdel{by fitting the }\DIFdelend \DIFaddbegin \DIFadd{from calculated }\DIFaddend $\Delta \Delta G$'s \DIFdelbegin \DIFdel{to the known }\DIFdelend \DIFaddbegin \DIFadd{while fitting a constant offset to best reproduce the }\DIFaddend experimental binding free energies \DIFaddbegin \DIFadd{for known compounds}\DIFaddend ~\cite{wang2015accurate}. \DIFdelbegin \DIFdel{This prompts us to consider another important aspect.}\DIFdelend \DIFaddbegin \DIFadd{Each $\Delta \Delta G$ is associated with a particular free energy calculation or transformation, which can be thought of as an edge in the graph spanning the compound series, see examples of such graphs in Fig.~\ref{fig:fig_types_of_networks}.
}

\DIFadd{The fact that we can analyze both $\Delta G$ values and $\Delta \Delta G$ values raises an important question about analysis -- which calculated values should we assess? }\DIFaddend It is important to be clear on what error to report: $\Delta G$ after shifting by a constant to minimize the RMSE, unshifted $\Delta G$, $\Delta \Delta G$ of computed edges, or $\Delta \Delta G$ of all edges. (See recommendations for reporting best practices, Sec.~\ref{sec:plot_data}.) Additionally, as it is possible to perform calculations on a set of ligands using different pairwise comparisons of molecules, the performance of the method may be biased based on which pairs of comparisons are performed. Additionally, it is possible that the error associated \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend the relative free energy between a two ligands that was not directly computed (however can be deduced using thermodynamic paths involving other ligands) will likely be more uncertain\DIFdelbegin %DIFDELCMD < \url{https://github.com/jchodera/jacs-dataset-analysis}%%%
\DIFdelend .
...
\DIFdelbegin \DIFdel{Finite }\DIFdelend \DIFaddbegin \DIFadd{We restrict our analysis here to repetitions of the same calculation performed with precisely the same force field - different force fields used to describe the same molecule can lead in some cases to wide difference in free energies.  Even within this restriction,
finite }\DIFaddend computing resources necessarily limit the generated number of uncorrelated samples of potential energy surfaces, and therefore alchemical free energy calculations only give free energy estimates to within finite precision. An important consideration is how reproducible alchemical free energy calculations are in practice. In simple cases such as absolute hydration free energies of small organic molecules, or relative hydration free energy calculations between structurally similar small organic molecules, it should be possible to obtain highly precise estimates with a given software package (with a sample standard deviation under 0.01 kcal/mol)~\cite{rizzi2019sampl6}.
For more complex use cases such as protein-ligand binding free energies the repeatability is often substantially worse~\cite{rizzi2019sampl6}. A good practice is to perform two or three runs of the same perturbation to assess precision with a given protocol\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, using different initial conditions (i.e. velocities). }\DIFaddend The sample standard deviation will give a crude estimate of the reliability of the estimates, and whether the precision is sufficient for the problem at hand. When practical, a more stringent test is to use different input coordinates for each repeat run. 
...
\begin{equation}
  K\DIFdelbegin \DIFdel{_D }\DIFdelend \DIFaddbegin \DIFadd{_b }\DIFaddend = K_D^\ast \frac{1 + K_C}{K_C}
...
distribution. That is, assume that the standard deviation in the relative binding free \DIFdelbegin \DIFdel{enrgies }\DIFdelend \DIFaddbegin \DIFadd{energies }\DIFaddend for the changes represented is $RT\ln 5$
...
If the accuracy of \DIFaddbegin \DIFadd{a collection of }\DIFaddend free energy calculations is $\sigma = 1$ kcal/mol, $P(\Delta\Delta
...
In summary, the successful use of alchemical calculations not only in, but particularly for drug discovery requires working in the domain of applicability, using a high quality X-ray structure of the target bound to compounds in the series, and testing the approach retrospectively to ensure the system setup is well-behaved. Always assess your confidence in the resulting predictions and communicate this when discussing with experimentalists. Consider performing repeat calculations for at least some of the perturbations in the study. 
 There are many accounts of success of alchemical calculations, the methods show good performance towards the goal of binding \DIFaddbegin \DIFadd{free }\DIFaddend energy prediction. However, it is important to have realistic expectations. 

Structure based drug design projects are often capable of improving potency relatively quickly, even with only limited application of computational approaches and the range of activity narrows to just two-to-three log units. It may seem hard to have impact with substantially different, more potent, stand-out compounds in this scenario, but binding \DIFdelbegin \DIFdel{energy prediction }\DIFdelend \DIFaddbegin \DIFadd{free energy predictions }\DIFaddend can still be extremely useful for ensuring activity is maintained as other properties are optimized. An interesting cost benefit analysis has shown the value of activity prediction, see discussion above and articles such as~\cite{mobley2012perspective}. 
...
In principle, in the limit of sufficient \DIFdelbegin \DIFdel{conformational }\DIFdelend \DIFaddbegin \DIFadd{configurational }\DIFaddend sampling, the free energy changes estimated from an alchemical free energy calculation should be independent of the system's initial coordinates. However, in practice, because simulations are of finite duration (typically 1-100 ns per state at present), this is only true for certain classes of alchemical free energy calculations such as relative or absolute free energies of hydration of small and relatively rigid organic molecules. Protein-ligand complexes typically exhibit slowly relaxing degrees of freedom that significantly exceed the duration of an alchemical free energy calculation, and host-guest calculations can be susceptible to these issues as well, depending on timescale and system. It is therefore generally important to carefully select input coordinates to obtain satisfactory results. 
...
\paragraph{Have you considered \DIFdelbegin \DIFdel{sterioisomers }\DIFdelend \DIFaddbegin \DIFadd{stereoisomers }\DIFaddend and enantiomers?}
...
A critical first step in relative calculations is to select \DIFdelbegin \DIFdel{an approach to these calculations, determining whether to use }\DIFdelend \DIFaddbegin \DIFadd{the approach to be used for these calculations and determine what this means for atom mapping (though this may sometimes be dictated by the requirements of the chosen simulation package. Sometimes this approach is described in terms of }\DIFaddend a \emph{dual topology} \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{versus }\DIFaddend \emph{single topology} \DIFdelbegin \DIFdel{, or }\emph{\DIFdel{hybrid topology}} %DIFAUXCMD
\DIFdelend approach to relative calculations.

The distinction between \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{single and dual topology }\DIFaddend can be illustrated by considering a hypothetical transformation from molecule A to molecule B, where both atoms share a common substructure but differ in their substituents; e.g. consider a transformation of benzene to benzyl alcohol Fig.~\ref{fig:fig_topology}.
...
In contrast, in a dual topology alchemical free energy calculation, no atoms are allowed to change type~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{shirts2012best}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Boresch:1999:J.Phys.Chem.A, shirts2012best}
}%DIFAUXCMD
}\DIFaddend . This means that the benzene to benzyl alcohol transformation involves starting with benzene plus the non-interacting dummy atoms making up the hydroxy methyl group, then passing through an intermediate state where some atoms are partially interacting--- particularly, those atoms which are becoming dummy atoms or ceasing to be dummy atoms~\cite{mobley2014blind}. The transformation finally culminates in a state where benzyl alcohol is present along with the additional dummy atom which was previously a corresponding hydrogen of the benzene. Fig.~\ref{fig:fig_topology}'s right branch depicts how such a dual topology works. 
\DIFaddbegin 

\DIFaddend \begin{figure}
    \includegraphics[width=0.95\columnwidth]{figures/fig3_topol/Figure.pdf}
    \caption{\textbf{Two common topologies for alchemical calculations: single and dual topology.} \textbf{Left}: A single topology uses softcore potentials to convert from one type of atom to an other. Dummy atoms (Du) are used when there is no corresponding maximum common substructure match between the two molecules for certain atoms. \textbf{Right}: The dual topology does not convert one species to another, but only converts between Du atoms and an interacting species, but usually uses softcore potentials for this. The 'mixed' intermediate atoms \DIFaddbeginFL \DIFaddFL{(M) }\DIFaddendFL are used in both dual and single topology approaches. Only the way the transformation occurs and the end states differ. Following the arrow along the left and right illustrate the differences. Figure adapted from \url{http://www.alchemistry.org/wiki/Constructing_a_Pathway_of_Intermediate_States}}
    \label{fig:fig_topology}
\end{figure} 

\DIFdelbegin \DIFdel{Hybrid topology calculations have seen much less usebut }\DIFdelend \DIFaddbegin \DIFadd{As far as we are aware, the distinction between single and dual topology approaches is made primarily for two main reasons. First, this choice affects the alchemical pathway followed, and thus may affect convergence properties -- though we are not yet aware of a study of the relative efficiency and merits of these two approaches. Additionally, historically, some simulation packages one approach and not the other, meaning that the distinction was functional. 
}

\DIFadd{Some additional terms are also employed. Particularly, some studies refer to a ``hybrid'' topology approach to free energy calculations~\mbox{%DIFAUXCMD
\cite{gapsys2015pmx, gapsys2016accurate, gapsys2020large}
}%DIFAUXCMD
, though this term may not have yet achieved widespread use. In this case, "hybrid" seems to indicate that the set up of these free energy calculations involves a hybrid of the two molecules, and much of what is done in these studies uses a single topology approach~\mbox{%DIFAUXCMD
\cite{gapsys2020large}
}%DIFAUXCMD
.
}

\DIFadd{One final approach, so far in its infancy, has been called ``separated topologies'' and }\DIFaddend essentially consist of two absolute free energy calculations in opposite directions at the same time, turning one molecule's interactions with the environment off, while turning the other molecule's interaction on~\cite{jiang2019computing, rocklin2013separated}.

\DIFdelbegin \DIFdel{Most existing software implementations currently }\DIFdelend \DIFaddbegin \DIFadd{Software packages vary in their }\DIFaddend use single or dual topology approaches; for example, AMBER TI uses a dual topology approach, while BioSimSpace uses a single topology approach. Please make sure to check what approach is used with your software package of choice, or whether it supports your choice of approach (GROMACS \DIFaddbegin \DIFadd{and GROMOS}\DIFaddend , for example, \DIFdelbegin \DIFdel{supports }\DIFdelend \DIFaddbegin \DIFadd{support }\DIFaddend both). 
...
If no common substructure is shared, then alternative dual or \DIFdelbegin \DIFdel{hybrid }\DIFdelend \DIFaddbegin \DIFadd{separated }\DIFaddend topology free energy calculations are needed, where one would co-localize a pair of compounds in a binding site, exclude their interactions with one another, and compute the relative binding free energy by turning one molecule on from being dummy atoms while turning the other off.
...
\DIFdelbegin \DIFdel{Typical }\DIFdelend \DIFaddbegin \DIFadd{Some }\DIFaddend molecular dynamics engines are not set up to recognize this change, or at least not to correctly include contributions to the free energy from changing constraints/constraint length, so results for a transformation \DIFdelbegin \DIFdel{would usually }\DIFdelend \DIFaddbegin \DIFadd{can }\DIFaddend be erroneous.
At present the most general solution to this problem is simply to avoid the use of constraints (and thus use a smaller timestep if necessary, usually of around 1 fs) in any relative free energy calculation involving a transformation of a constrained bond. \DIFdelbegin \DIFdel{When using TI in GROMACS }\DIFdelend \DIFaddbegin \DIFadd{Individual software programs and settings can handle such issues. For example, bonds can be transformed in both GROMACS and GROMOS, because contributions of LINCS (GROMACS) and SHAKE (GROMOS) contrained bonds are added to the }\DIFaddend $\frac{dH}{d\lambda}$ \DIFdelbegin \DIFdel{calculations have contribution from the LINCS constrained bonds , thus transforming constrained bonds in GROMACS is possible}\DIFdelend \DIFaddbegin \DIFadd{term~\mbox{%DIFAUXCMD
\cite{pearlman1993determining, straatsma1992holonomica, pearlman1991overlooked, gunsteren1997computer}
}%DIFAUXCMD
. However, the constraints are not taken into account when calculating energy differences to other intermediate states (as the effect is entropic, not energetic). User manuals should carefully checked for how these effects are included if a constrained bond changes in length. We do note that if one performs calculations of the changing constraint in both solution and in the binding site, then the errors often cancel substantially, but the cancellation is not guaranteed}\DIFaddend .
...
It is convenient to define a standard state at a constant pressure of 1~atm and where each chemical species (i.e., \DIFdelbegin \DIFdel{A, B, and AB}\DIFdelend \DIFaddbegin \DIFadd{R, L, and RL}\DIFaddend ) in the reaction solvent has a concentration of $c^{\circ}$~=~1~M~=~1~molecule/1660~\r{A}$^3$ but do not interact with other molecules of $R$, \DIFdelbegin \DIFdel{$B$}\DIFdelend \DIFaddbegin \DIFadd{$L$}\DIFaddend , or $RL$.
...
\begin{equation}\label{eq:restraint-correction}
    \Delta G^{\circ}_{\mathrm{restr}} = -k_BT ~ \ln \left( \DIFdelbegin \DIFdel{\frac{c^{\circ}}{V_L} }\DIFdelend \DIFaddbegin \DIFadd{c^{\circ}{V_L} }\DIFaddend \right) -k_BT ~ \ln \left( \frac{\xi_L}{8 \pi^2} \right),
\end{equation}
...
When \DIFdelbegin \DIFdel{restraint }\DIFdelend \DIFaddbegin \DIFadd{restraints }\DIFaddend are employed, the values of $V_L$ and $\xi_L$ are restraint-dependent, but for commonly employed restraints, these can be usually easily computed analytically or numerically by solving the relevant integral.
...
The bias can be corrected through reweighting schemes~\cite{rizzi2019sampl6}, but this post-processing step can be avoided if a flat-bottom restraint is used and the ligand \DIFdelbegin \DIFdel{never hits }\DIFdelend \DIFaddbegin \DIFadd{is never sampled while hitting }\DIFaddend the potential wall during the simulation in the bound state \DIFaddbegin \DIFadd{as the numerical correction will be exactly zero}\DIFaddend .
...
In the latter case, \DIFdelbegin \DIFdel{to compute }\DIFdelend \DIFaddbegin \DIFadd{however, care must be taken when computing }\DIFaddend the standard state correction\DIFdelbegin \DIFdel{analytically, the bound-state restraint must be turned off in the decoupled state}\DIFdelend \DIFaddbegin \DIFadd{.
When multiple restraining potentials are active in the non-interacting state, the correction can generally be computed only through numerical integration}\DIFaddend .
Alternatively, \DIFdelbegin \DIFdel{a flat-bottom restraint can be activated also in the decoupled stateas long as the second restraint (e.
g., a harmonic or Boresch restraint) prevents the ligand to hit the wall of the flat-bottom potential~\mbox{%DIFAUXCMD
\cite{rizzi2019sampl6}
}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{one can adopt a protocol that removes the bound-state restraint in the non-interacting state.
...
\begin{equation}
U(\vec{q},\vec{\lambda}) = \DIFaddbegin \DIFadd{(1-}\DIFaddend \vec{\lambda}\DIFaddbegin \DIFadd{) }\DIFaddend U_0(\vec{q}) + \DIFdelbegin \DIFdel{(1-}\DIFdelend \vec{\lambda}\DIFdelbegin \DIFdel{)}\DIFdelend U_1(\vec{q}) \end{equation}
...
However, this approach ends up being terrible for removing or adding repulsive potentials that go to infinity quickly at or near the origin. One way to look at this is to examine how low $\vec{\lambda}$ values must go to reduce the energy at $0.5\sigma$ (the atomic size parameter) down to 1 $k_BT$, where thermal fluctuations make it possible for other atomic sites to penetrate routinely that deep. \DIFdelbegin \DIFdel{If }\DIFdelend \DIFaddbegin \DIFadd{Assume }\DIFaddend we are trying to go from a particle being present, and desire to make it disappear alchemically\DIFdelbegin \DIFdel{, then if }\DIFdelend \DIFaddbegin \DIFadd{. If }\DIFaddend the repulsive terms are of the form \DIFdelbegin \DIFdel{$\epsilon\frac{\sigma}{r}^{12}$, then }\DIFdelend \DIFaddbegin \DIFadd{$\epsilon(\frac{\sigma}{r})^{12}$, and }\DIFaddend if $\epsilon$ \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend 1 $k_BT$ at the temperature of interest, and we start with the particle present, \DIFdelbegin \DIFdel{then solving for $(1-\vec{\lambda})(1 kB_T)\left(\frac{\sigma}{\frac{1}{2}\sigma}\right)^{12} = 1 k_B T$ we get }\DIFdelend \DIFaddbegin \DIFadd{we may solve for $(1-\vec{\lambda})(1 k_B T)\left(\frac{\sigma}{\frac{1}{2}\sigma}\right)^{12} = 1 k_B T$. This yields }\DIFaddend $\vec{\lambda} = 1-2^{-12} \sim 0.999976$. At this point,  we have gone virtually all the way to the end of the transformation, but there is still an impenetrable post in the middle of our simulation! This is not very much like the desired final state of no interactions between the particle and its environment. We can play around with a few ways of modifying this, like simulating many more intermediate states near $\vec{\lambda}=1$. However, various analyses have shown that this is not a very good strategy~\cite{pham2011identifying, beutler1994avoiding, zacharias1994separationshifted, blondel2004ensemble, gapsys2012new}.

What we need instead is a function that smoothly gets rid of this infinity. A large number of schemes have been tried~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{beutler1994avoiding, zacharias1994separationshifted, blondel2004ensemble, pham2011identifying, pham2012optimal, naden2014linear, donnini2005molecular}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{beutler1994avoiding, zacharias1994separationshifted, blondel2004ensemble, pham2011identifying, pham2012optimal, naden2014linear, donnini2005incorporating}
}%DIFAUXCMD
}\DIFaddend , but the most common strategy that appears to be the best practice is to use a "soft-core" potential, of the form:
...
Other schemes, such as simultaneously changing electrostatic and Lennard-Jones interactions with electrostatic ``soft-core'' potentials~\cite{steinbrecher2007nonlinear} may be implemented with fewer intermediate \DIFaddbegin \DIFadd{states }\DIFaddend but could require fine-tuning of electrostatic and Lennard-Jones softcore parameters to avoid numerical instabilities. 
...
\begin{figure}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.8\linewidth]{paper/figures/fig_pmf/pmf.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.8\linewidth]{figures/fig_pmf/pmf.png}
    \DIFaddendFL \caption{{\bf Potential of mean force with respect to $\vec{\lambda}$ for TI and MBAR}
...
The uncertainty in the free energy, for example, \DIFdelbegin \DIFdel{has multiple ways to be estimated }\DIFdelend \DIFaddbegin \DIFadd{can be estimated in multiple ways}\DIFaddend , e.g. through standard error propagation methods (including MBAR's estimator, which is based on the same principles as standard error propagation), through bootstrap methods, through multiple independent runs, etc. 
...
\DIFaddbegin \DIFadd{In general, uncertainties can be assumed to decrease as $1/\sqrt(N)$ where $N$ is the number of uncorrelated samples, for all standard free energy calculation methods~\mbox{%DIFAUXCMD
\cite{shirts2005comparison}
}%DIFAUXCMD
. However, this carries the notable caveat that such estimates require accurate estimation of the correlation time which, if important motions are slow compared to simulation timescales, can be difficult. Still, this metric provides a good rule of thumb, and as long as conformational transitions are captured by the simulation, increasing the aggregate simulation time by a factor of $T$ will reduce the uncertainty by a factor of approximately $\sqrt(T)$.
}

\DIFaddend \subsubsection{Multiple or uncertain binding modes may require considerable care}
...
Sampling all potential \DIFdelbegin \DIFdel{lignad }\DIFdelend \DIFaddbegin \DIFadd{ligand }\DIFaddend binding modes within a single free energy calculation is usually impractical without some form of enhanced sampling or at least Hamiltonian replica exchange~\cite{wang2013identifying} because barriers for binding mode interconversion result in kinetics which are too slow compared to simulation timescales~\cite{mobley2006use, palma2012computation,mobley2012perspective, gill2018binding}.
...
Once \DIFaddbegin \DIFadd{equilibrium }\DIFaddend data has been collected from alchemical intermediates, it must be analyzed to produce an estimate of the free energy change (and its associated statistical uncertainty) for each leg of the thermodynamic cycle.
...
In such cases, this approach simply selects the final portion of the the simulation, which may be contained in a single substate of \DIFdelbegin \DIFdel{conformational }\DIFdelend \DIFaddbegin \DIFadd{configurationaal }\DIFaddend space, and may itself lead to biased estimates. 
...
If coupled simulations are used (such as a Hamiltonian replica exchange simulation), the replicas should undergo equivalent random walks in alchemical space, and the replicas can be \DIFdelbegin \DIFdel{can be }\DIFdelend subsampled with the same $g$ to generate an equal number of uncorrelated samples at each alchemical state.
...
Described in words, we take the samples generated during our run with the potential energy function $u_0(\vec{q})$ and calculate what the difference in energy would be if we switched to potential energy function $u_1(\vec{q})$, and average the exponential of the negative energy difference to get the negative of the exponential of the free energy difference. The original distributions, P($u_0$) as generated at $\vec{\lambda}=0$ and P($u_1$) would look like those seen in Fig.~\ref{fig:fig_sampling_scheme} \textbf{A}-\textbf{C} on the \DIFdelbegin \DIFdel{left }\DIFdelend \DIFaddbegin \DIFadd{right }\DIFaddend hand side. Reevaluating requires almost no extra code functionality to perform; one need only to save a full precision trajectory, and run an unmodified molecular simulation code using the $u_1$ in order to calculate the new energies of stored snapshots. The analysis can be written in a line of code. We note that this method is even more general, in that the instantaneous work to change the potential energy function from $u_0$ to $u_1$ can be replaced by the non-reversible work $W$ to make the same change \DIFdelbegin \DIFdel{under }\DIFdelend \DIFaddbegin \DIFadd{beginning from }\DIFaddend the same equilibrium conditions at either end state~\cite{jarzynski1997nonequilibrium,jarzynski1998equilibrium,crooks2000pathensemble}, \DIFdelbegin \DIFdel{though we do not go into all of the details of }\DIFdelend \DIFaddbegin \DIFadd{allowing for }\DIFaddend non-equilibrium \DIFaddbegin \DIFadd{free energy calculations, an alternate approach. We do not detail non-equilibrium }\DIFaddend transformations here, and refer the reader to more advanced treatments~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{maragakis2008bayesian,oberhofer2005biased,procacci2015unbiased,shirts2003equilibriuma,ytreberg2004singleensemble}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{maragakis2008bayesian,oberhofer2005biased,procacci2015unbiased,shirts2003equilibriuma,ytreberg2004singleensemble, gapsys2020large}
}%DIFAUXCMD
, as our focus here is on equilibrium free energy techniques}\DIFaddend .
...
\begin{eqnarray}
 ...
 =\sum_{i=1}^{n_j} \DIFdelbegin \DIFdel{\frac{1}{1 + \exp[\ln(\frac{n_i}{n_j}) - u_{ij}(\vec{q}) + f_ij)]}}\DIFdelend \DIFaddbegin \DIFadd{\frac{1}{1 + \exp[\ln(\frac{n_i}{n_j}) - u_{ij}(\vec{q}) + f_{ij})]}}\DIFaddend ,
\end{eqnarray}
...
\end{equation}
And \DIFdelbegin \DIFdel{than we can then }\DIFdelend \DIFaddbegin \DIFadd{then we can }\DIFaddend numerically integrate $df/d\vec{\lambda}$ over an alchemical transformation, using a range of different well-established techniques, to obtain:
\begin{equation}
...
\item TI usually gives similar values as MBAR implemented with sufficient numbers of intermediates, but quadrature errors \DIFaddbegin \DIFadd{that }\DIFaddend are hard to estimate beforehand  can occur if one is not careful.~\cite{paliwal2011benchmark}
...
In this case, one can extend the }\DIFaddend simulations or try an approach that requires simulations in two separate binding modes where they interconvert at very slow timescales.  
\begin{figure}
    \includegraphics[width=0.90\linewidth]{figures/fig9_convergence/Figure.pdf}
    \caption{Average binding free energy of 5 replicate Hamiltonian replica exchange calculations as a function of total simulation time (i.e. the sum of the simulation time of all replicas) for the two host-guest systems CB8-G3 and OA-G3. Shaded areas represent 95\% confidence intervals around the mean computed from the 5 replicates data. The horizontal \DIFdelbeginFL \DIFdelFL{dashed }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{dash-dot }\DIFaddendFL lines show the final binding free energy prediction of the two calculations after a total of 5230 ns for OA-G3 and 6650 ns for CB8-G3. \DIFaddbeginFL \DIFaddFL{Dashed lines are the free energy trajectories computed in the forward (blue) and reverse (red) time direction for a single replicate calculation. }\DIFaddendFL Longer correlation times in CB8-G3 cause the calculation to converge more slowly. The original data used to generate the plot can be found at \url{https://github.com/samplchallenges/SAMPL6/blob/master/host_guest/Analysis/SAMPLing/Data/reference_free_energies.csv}.
}
...
\item \textbf{Dependence of the free energy on initial \DIFdelbegin \DIFdel{conformation }\DIFdelend \DIFaddbegin \DIFadd{configuration }\DIFaddend of the system.} Ensemble average properties should not depend on the starting point.
\item \textbf{Torsional sampling} Torsions with multiple low-energy minima where some of these minima are \DIFdelbegin \DIFdel{that are }\DIFdelend visited rarely or not at all. Which torsions have low energy minima can best be found by comparing to the simulation in the solvent. There should be clear physical reasons that simulation in the complex has different torsional distributions that the ligand in the solvent. 
...
\item \textbf{Convergence of free energy} The free energy should clearly converge as a function \DIFdelbegin \DIFdel{or }\DIFdelend of simulation time (Fig. ~\ref{fig:convergence_forward_reverse}).
...
\label{sec:software}
There are many different software solutions available for the setup, running, and analysis of \DIFdelbegin \DIFdel{alchemcial }\DIFdelend \DIFaddbegin \DIFadd{alchemical }\DIFaddend free energy calculations. These will vary in customizability and ways in which they are ran, e.g. graphical user interface versus command line tool or python script. The following provides a non-exhaustive list of commercial and noncommercial tools available for conducting alchemical free energy calculations. 
\begin{itemize}
...
\item [] \textbf{Simulation software: Free/low cost academic and Commercial}
	...
	\DIFaddbegin \item \href{http://www.gromos.net/}{GROMOS} \DIFadd{offers an extensive and flexible molecular dynamics and simulations analysis suites with free energy calculation functionalities including customizable alchemical paths and various sampling protocols~\mbox{%DIFAUXCMD
\cite{schmid2012architecture, kunz2012new, eichenberger2011gromos}
}%DIFAUXCMD
.
	}\DIFaddend \end{itemize}
\item [] \textbf{Simulation software: Open Source}
	...
	\item \href{http://pmx.mpibpc.mpg.de/instructions.html}{PMX}, an add-on to GROMACS, offers a mutation free energy calculation module\DIFaddbegin \DIFadd{~}\DIFaddend \cite{abraham2015gromacs}.
	\item \href{https://github.com/qusers/Q6}{Q} is MD code for performing FEP calculations using a variety of force fields~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{aqvistjohan2017q6}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{aqvistj.2017q6}
}%DIFAUXCMD
...
\item[] \textbf{Setup tools:}
	\begin{itemize}
	\item \href{http://pmx.mpibpc.mpg.de/instructions.html}{PMX}: \DIFdelbegin \DIFdel{GROMACS }\DIFdelend \DIFaddbegin \DIFadd{Setup of perturbation maps, perturbed topologies and input coordinates for GROMACS simulations}\DIFaddend \url{https://github.com/deGrootLab/pmx}.
...
\begin{itemize}
   \item Check your ligands  have the same, biologically correct binding pose
        \item Make sure your \DIFdelbegin \DIFdel{λ}\DIFdelend \DIFaddbegin \DIFadd{$\lambda$}\DIFaddend -scheduling is set correctly
...
\begin{itemize}
    ...
    \item Inspect the trajectories across the \DIFdelbegin \DIFdel{λ}\DIFdelend \DIFaddbegin \DIFadd{$\lambda$}\DIFaddend -schedule (particularly the endpoints) for problems described in the text
...
\DIFaddbegin \textbf{\DIFadd{SP}} \DIFadd{Wrote Sec.~\ref{sec:software}.
}\textbf{\DIFadd{DFH}} \DIFadd{Contributed to Sec.~\ref{sec:software} and helped edit the paper.
}\DIFaddend % We suggest you preserve this comment:
...
JDC is a current member of the Scientific Advisory Board of OpenEye Scientific Software and a consultant to Foresite Laboratories. A complete funding history for the Chodera lab can be found at \url{http://choderalab.org/funding}. DLM \DIFdelbegin \DIFdel{acknowledges }\DIFdelend \DIFaddbegin \DIFadd{appreciates financial }\DIFaddend support from the National Institutes of Health (\DIFdelbegin \DIFdel{R01GM108889, R01GM124270, }\DIFdelend \DIFaddbegin \DIFadd{1R01GM108889-01, 1R01GM124270-01A1 }\DIFaddend and R01GM132386), and the National Science Foundation (CHE 1352608)\DIFdelbegin \DIFdel{.
JDC acknowledges support from the National Institutes of Health (NIH P30 CA008748, NIH R01 GM121505, R01 GM132386).
The content is solely the responsibility of the authors and does not necessarily represent the official views of the National Institutes of Health}\DIFdelend .
